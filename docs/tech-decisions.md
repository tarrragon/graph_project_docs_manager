...
**append-only 的一項例外：失效標記屬既有段落的合法加註，不視為改寫。** 當某結論的前提
已被後續補記推翻時，允許在該結論**之後**加註一行 `**前提已失效**（… 見〈補記段標題〉）`，
原結論文字一字不動。此例外為必要——本檔無 status 欄、不受任何 CLI 掃描，讀者多以 grep
落在單行上，若失效訊息只存在於數百行外的補記段，grep 讀者沒有路徑會走到它。加註的是
指標，不是新的判斷；判斷仍只在補記段裡做。
...
**前提已失效**（2026-08-27 tripwire 觸發，見文末〈2026-08-27：已觸發的 tripwire —— 加入編輯能力〉）：
「首版唯讀」的決定已被推翻——票 `0.0.3-W1-002` 定案「本 App 直接改檔、CLI 僅作驗證器」，
這條線已經跨越，上句所稱的重要性不再是「維持唯讀」的理由，而是四項重評（備份／衝突／
還原／PII）必須執行的理由。重評由 `0.1.0-W3-072` 承接。
...
| 物件層級授權檢查 | 不適用 | 單使用者、本機、唯讀 —— **前提已失效**（2026-08-27 tripwire 觸發，見文末〈2026-08-27：已觸發的 tripwire —— 加入編輯能力〉）：「唯讀」不成立，App 現在會直接改使用者的檔案，本列的「不適用」不可再作為「不需授權檢查」的依據。重評見 `0.1.0-W3-072` |
| **輸入驗證** | **待實作** | 見下方 §發現二 |
| **資源上限** | **待實作** | 見下方 §發現二 |
...
| 自動備份 + 還原驗證 | 不適用 | App 不擁有任何資料，全部衍生自它不擁有的檔案 —— **前提已失效**（2026-08-27 tripwire 觸發，見文末〈2026-08-27：已觸發的 tripwire —— 加入編輯能力〉）：App 現在會改使用者的檔案，「不擁有資料」不再涵蓋寫入路徑，備份與還原重新適用。重評見 `0.1.0-W3-072` |
| 租戶隔離邊界 | 不適用 | 無多租戶 |
| PII 盤點 + log 遮罩 | **延後** | Stage 3 已定「破洞報告即診斷入口」，目前不寫執行期 log。**重評條件**：若日後加入 log，使用者的文件內容（可能含敏感資訊）會進 log，屆時須先做遮罩設計 —— **前提已失效**（2026-08-27 tripwire 觸發，見文末〈2026-08-27：已觸發的 tripwire —— 加入編輯能力〉）：本列的重評條件只綁「日後加入 log」，但寫入路徑本身就是新的 PII 面（App 會改寫可能含敏感資訊的使用者文件），不寫 log 也不消失，該條件因此不足以涵蓋現況。重評見 `0.1.0-W3-072` |

> 「App 不擁有資料」同時是一項安全紅利：首版唯讀，因此不存在破壞使用者
> 資料的路徑。加入編輯能力即跨越這條線——見 tripwire 總表。
>
> **前提已失效**（2026-08-27 tripwire 觸發，見文末〈2026-08-27：已觸發的 tripwire ——
> 加入編輯能力〉）：該安全紅利已消失。上句「加入編輯能力即跨越這條線」寫的是未來式，
> 但這條線已於票 `0.0.3-W1-002` 跨越；破壞使用者資料的路徑現在存在。上句指向的
> tripwire 總表記的是「尚未觸發」的形態，須改讀文末補記段。重評見 `0.1.0-W3-072`。
...
| 觸發條件 | 影響 | 動作 | 觸發偵測的承接文件／ticket |
|---------|------|------|--------------------------|
| 決定上架 Mac App Store | 沙盒必須開啟，`Process.run` 失去執行 CLI 能力 | 回頭重評 PROP-001，重新引入 security-scoped bookmark | `test/entitlements_contract_test.dart`（沙盒改開即紅燈，逼出 §2.1）。決策階段本身仍無偵測者 |
| **加入編輯能力** | 跨越「App 不擁有資料」的界線 | 備份、衝突、還原、PII 全部重評；等同新的架構決策 —— **【此條已於 2026-08-27 判定觸發，action 已落票 `0.1.0-W3-072`】** 見文末「已觸發的 tripwire」 | `0.1.0-W3-072`（四項重評） |
| 上游 `0.2.1-W3-1110` 定案 | 部分 `proposed` 型別升為 `established` | 重評 proposed 型別的渲染策略；`flow 未結構化` 的出現率下降（SPEC-001 FR-06） | **待指名**（PROP-002 §Tripwire 是同一條的複述，同樣無 status 欄、不受掃描，不構成偵測者） |
| 上游 `0.2.1-W3-1115` 定案 | `where.layer` 的處置方式確定 | 重評 ticket → domain 的定位機制（PROP-004） | **待指名**（PROP-004 §Tripwire 同上，為複述非偵測） |
| 上游 `id_pattern` 加入 lookbehind 或 `\p{}` | Dart `RegExp` 語意不符 | 契約測試會紅燈；須調整解析或請上游改寫 | **待指名**（動作欄假設的契約測試尚不存在：`test/` 下僅 entitlements 與 window size 兩支契約測試） |
| 專案檔案數超過萬檔級 | 依視圖惰性載入的假設仍成立，但單次載入時間拉長 | 重評是否需要持久化快取（目前明確不做） | **待指名** |
| 出現無法由破洞報告解釋的故障 | 診斷入口不足 | 重評 structured log 的延後決定 | **待指名**（`0.1.0-W1-013` 裁定維持延後並保留本條，但該票已完成，未指派偵測者） |
| 使用者回報誤選大目錄導致當機 | 資源上限缺失 | 見發現二，該防護應在此之前完成 | **待指名**（發現二的「資源上限」是防護，不偵測本條觸發） |

**「觸發偵測的承接文件／ticket」欄的判準**：填入的必須是「觸發條件成立時會主動發聲」的
載體——會紅燈的測試、會被執行的 ticket、會擋下操作的 hook。**複述同一條 tripwire 的散文
段落不算**：它與本表同為 append-only 且無人掃描，把它填進來只會讓本欄變成第二層假覆蓋，
與本表原本的問題同型（見文末〈2026-08-27：已觸發的 tripwire —— 加入編輯能力〉§為什麼沒有
人發現）。八條中六條為「待指名」，這是本欄第一次量到的真實覆蓋率，不是待辦清單的登記
遺漏；指名工作不在補欄的票內，另行處置。
...
> **處置已執行**（`0.1.0-W3-062`，2026-09-08）：總表已補「觸發偵測的承接文件／ticket」欄
> 並逐條填寫（八條中六條為「待指名」，指名工作未在該票內做）；四項重評落為
> `0.1.0-W3-072`，編輯能力規格落為 `0.1.0-W3-073`。上表五處失效結論亦已於**原處**
> 各加註失效標記——本節原本只是單向指向它們，grep 落在那五行上的讀者走不到這裡。
...
### 2026-09-03：外部開啟原始檔與目錄在 0.1 落地（W1-036）

SPEC-001 §1、§2、§5、§6 四處有「開啟原始檔」或「開啟 docs 目錄」動作，SPEC-003 §3
為其定義了反應，但 0.1 是否實際落地無決策記錄；依 SPEC-003 FR-06，懸空會讓畫面票
無法決定元件要承載幾種形態。

**裁決：0.1 落地**，不採 disabled + 常駐說明。

| 判準 | 事實 |
|------|------|
| 可行性 | 沙盒關閉（PROP-001）；裸 dart 程序實測 `/usr/bin/open <file>`、`open <dir>` 皆 `exit=0`（macOS 26.5、Flutter 3.47.1）。簽章 `.app` 內未實測，列為 W1-068 的實機驗收項 |
| 失敗訊號 | 檔案不存在 `exit=1` + stderr「does not exist」；無應用程式對應 `exit=1` + `kLSApplicationNotFoundErr`。實作以 `FileSystemEntity.type` 前置判定不存在，不解析 stderr |
| 可測性（本票的判準） | 外部程序呼叫以可注入抽象 `ExternalOpener` 的 fake 呼叫紀錄斷言（SPEC-003 概述表既已宣告此斷言方式，即規格本身預設落地） |
| 依賴 | 不新增：`url_launcher_macos` 等價 `NSWorkspace.open(URL)`，同為成功／失敗二值 |
| 機會成本 | 不落地須寫四處 disabled 文案 + i18n key，之後落地時再回頭改四處；落地成本為一個抽象 + 一個新 key，且規格已預設此路徑 |

**收窄**：§3.5 破洞項「定位至行號」0.1 不做——`open` 無行號參數，定位需編輯器專屬
URL scheme 與偵測，另立 W1-070 追蹤 SPEC-001 §5 的承諾。
...
### 2026-09-09：接縫形態約定與 SPEC-005 的軸別對照（W3-157）

本段為補記段「2026-09-09：driven port 接縫形態約定（W3-133／W3-146）」（下稱
「形態約定段」，以標題引用，本檔 append-only 故不引行號）與
`docs/spec/workspace/SPEC-005-workspace-driven-ports.md` 的交叉註記，不改寫形態
約定段任何結論。兩份文件描述同一組接縫但切分方式與收錄範圍不同，缺此註記時交叉
比對者會判為矛盾並試圖修正其中一方；SPEC-005 §1 與 §1.1 已有對應方向的註記，本段
是另一個入口的同一份說明。

**兩處的「列」不是同一種東西**：形態約定段的「四個接縫現況對照」把
`WorkspacePreferencesPort` 與 `WorkspacePreferencesHandle` 放在同一列，SPEC-005
§1 的接縫清單則分列。兩邊都不是待修正狀態——

| | 形態約定段對照表的列 | SPEC-005 §1 的列 |
|---|---|---|
| 列 = 什麼 | 形態判準的**計算單位** | 回饋點承擔者的**歸屬單位** |
| 為何如此切 | 判準一算的是「port 方法數 ＋ 其 handle 方法數」的操作總數，兩者的操作數必須合計才得出形態 | `Handle` 承擔結果時刻、`Port` 承擔受理時刻，兩者須分別記載（見該規格 §2.2） |

**形態約定段這一列若被拆成兩列，判準會給出相反結論**：併列時的計算是 `open` 1 個 ＋
`readString`／`writeString` 2 個 ＝ 總數 3，落在「總數 >= 2 → `abstract interface
class`」。拆開後 `WorkspacePreferencesPort` 單獨只有 `open` 一個方法，總數 = 1，
落進「總數 = 1 → typedef 函式型別」那一支——本專案最複雜的接縫會被判成函式型別。
形態約定段內文原有一句「當總數分佈於 port 與 handle 兩個物件時，兩者一併採介面
class」講的是這個結論，未講拆開計數會得到相反結論，本段補的是後者。

拆列的動機通常是「對齊 SPEC-005、消除不一致」，看起來正當，實際是拆掉了判準計數
的定義域。**因此：不得依 SPEC-005 的粒度去改形態約定段的分組，亦不得依該段的分組
去合併 SPEC-005 §1 的列。**

**`WorkspaceLogSink` 兩處收錄狀態不同亦非矛盾**：形態約定段判準表收錄它並給出第 3 列
形態（`typedef` ＋ 自撰 private 頂層函式 `_defaultLogSink`）——判準輸入為操作總數
1，該計數不觸發任何遞迴。SPEC-005 §1.1 將它排除於**回饋點盤點**之外，理由是可觀
測性義務軸上的判準遞迴（記錄它自己的呼叫發出，投影載體仍是它自己）。兩者分屬介
面形態軸與可觀測性義務軸，結論可並存。**具體要擋的推論**：見 SPEC-005 §1.1 的排
除即把形態約定段判準表的 `WorkspaceLogSink` 一列刪去——那會刪掉第 3 列形態唯一的
現況實例，使下游三票套用判準一時無例可循。

**形態約定段一處時態已過期，不改寫，於此補正**：該段「四個接縫現況對照」前的定位
說明寫「行號會因 `0.1.0-W3-147` 提取 value types 而位移，不引用」。該票已合併進
`origin/main`，位移已發生；「依符號名定位、不引用行號」的做法不變，僅該句所述的
待發生事件現為已發生事實。value types 現位於 `lib/workspace/workspace_types.dart`
（九個型別），`lib/workspace/workspace_repository.dart` 以 import 引用且不
`export` 之，三個 port 介面、其 private `_Default*` adapter、兩個 typedef 皆留在
後者（2026-09-09 實查）。


## 2026-09-10：加入編輯能力 tripwire 的四項重評（`0.1.0-W3-072`）

tripwire 總表「加入編輯能力」規定的 action 為備份／衝突／還原／PII 全部重評。
本節逐項給出結論。**另補第五項授權檢查**——它列於〈已觸發的 tripwire〉五處失效
前提之中，與四項同源於同一條跨線，分開處理會使該處失效標註無新結論可指。

### 事實基礎：跨線發生在決策層，實作層尚未

重評前先查證跨線的實際狀態，不沿用既有標註的轉述：

| 層 | 狀態 | 依據（2026-09-10 實查） |
|------|------|------|
| 決策 | 已跨線 | 票 `0.0.3-W1-002` 定案「本 App 直接改檔、CLI 僅作驗證器」，`domain-map.md` §4.4 |
| 規格 | 未跨線 | 編輯能力規格尚未撰寫，落於 `0.1.0-W3-073`（待處理） |
| 實作 | 未跨線 | `lib/` 全域的檔案系統操作僅三處且皆唯讀：`Directory.exists`、`Directory.list`、`File.existsSync`。`writeAsString` / `writeAsBytes` / `openWrite` / `delete` / `rename` 命中數為 0。`shared_preferences` 存的是 App 自身選定的工作區路徑，非使用者內容 |

**既有五處失效標註的時態需就地理解為決策層，不是實作層**：那五處寫「App 現在會直接
改使用者的檔案」「破壞使用者資料的路徑現在存在」。以實作層讀之為假——今日不存在該
路徑。以決策層讀之為真，且正是它們要表達的意思：作為論證前提的「唯讀」已不可再用。
本節不改寫那五處（append-only），於此補正其射程。

**這個差別改變本次重評的性質，故非細節**：五項不是對既成事實的事後補救，而是**在寫入
路徑存在之前**先行成立的設計約束。它們的落地載體因此是 `0.1.0-W3-073` 的規格條文，
而非事後的修補票——條文先於程式碼存在時，違反它需要有人刻意寫出違反的程式碼；反過來
則需要有人回頭發現並改掉已經動作的東西。

### 貫串五項的原則：App 不成為使用者內容的第二個持有者

原「App 不擁有資料」是一項安全紅利，跨線後不能原樣保留。但它可拆為兩件事，跨線只
消滅了其中一件：

| 原紅利的成分 | 跨線後 |
|---|---|
| App 不修改使用者內容 | **已消滅**，不可回復——編輯能力的定義即是修改 |
| App 不持有使用者內容的第二份副本 | **仍可保留**，且是紅利中唯一還能保住的部分 |

**Why 值得刻意保住**：一旦 App 自建備份庫、undo 堆疊或變更日誌落地，使用者內容就有了
第二個持有者，而該持有者的保留期、清除時機、加密與否、是否隨 App 移除而消失全部成為
新的設計負擔與新的 PII 面。原本這些問題都不存在，不是因為解決了，是因為沒有第二份副本。

**Action**：下列五項的結論皆由此原則推得——凡需要「保存內容」的機能，一律外包給使用者
既有的 git，App 只負責「確認外包確實成立」與「讓使用者知道要還原什麼」。

### 一、備份：不自建備份庫，改為寫入前的可復原性閘門

**結論**：不建立 App 自有的備份儲存體。改為在每一次寫入前判定目標檔的可復原性，
三態如下，判定結果為寫入的前置條件而非事後提示：

| 目標檔狀態 | 可復原性 | 處置 |
|---|---|---|
| git 追蹤，且該檔無未提交變更 | 可復原 | 直接寫入。使用者的 git 即為本次寫入的備份 |
| git 追蹤，但該檔有未提交變更 | **不可復原** | 阻擋寫入 |
| 未被 git 追蹤，或工作區非 git 目錄 | **不可復原** | 阻擋寫入 |

**Why 不能只說「使用者的 git 即備份」**：git 只保護**已提交**的內容。對一個帶未提交
編輯的檔案寫入，被覆蓋的是那些未提交編輯，而 `git checkout` 只能還原到 HEAD，還不回
「五分鐘前」。未追蹤檔與非 git 工作區更是完全無保護。「git 即備份」在多數情況為真，
恰恰在資料真的會遺失的那三種情況為假——這正是靜默失效的形狀。

**Consequence（不做的後果）**：閘門缺席時，覆蓋未提交編輯不會有任何徵狀：寫入成功、
畫面正常、驗證器綠燈，使用者要到下次找那段編輯時才發現它不見了，且無法歸因到本 App。

**Action**：阻擋態沿用 SPEC-001 既有的阻擋狀態機制，不新增狀態類別；閘門的判定介面與
文案落 `0.1.0-W3-073`。阻擋後提供給使用者的出路（先提交／仍要寫入）屬互動設計，同票決定。

### 二、衝突：讀取時記錄內容指紋，寫入前重驗

**結論**：採樂觀併發控制。掃描讀入時記錄目標檔的內容雜湊，寫入前重新計算並比對，
不一致即判為外部已修改，阻擋寫入並要求重讀。不採檔案鎖。

**Why 不採檔案鎖**：macOS 的檔案鎖對外部編輯器無強制力——使用者用任何編輯器都能照常
寫入。設一個擋不住目標對象的鎖，只會讓實作者以為衝突已被處理。

**Why 用內容雜湊而非 mtime 加大小**：mtime 的精度與檔案系統相關，且「內容改回原樣」
與「內容未變」在 mtime 上不同而在語意上相同。目標檔為 markdown，體量使雜湊成本可忽略。

**Consequence**：無此機制時，App 以記憶體中的舊內容整檔寫回，使用者在外部編輯器所做的
修改被靜默覆蓋，且與第一項不同——即使該檔已提交、git 有備份，使用者仍不知道發生過覆蓋。

**Action**：指紋的記錄點與失效視窗（掃描到寫入之間）落 `0.1.0-W3-073`；阻擋態同第一項
沿用既有機制。

### 三、還原：路徑為使用者的 git，App 的義務是讓使用者知道要還原什麼

**結論**：不建 App 自有的 undo 堆疊。還原路徑即 git（`git diff` 檢視、`git checkout <file>`
還原）。但 App 須在每次寫入後產出「本次寫入改了哪些檔案、哪些欄位」的可見記錄。

**Why 記錄是必要的一半**：第一項的閘門保證每次成功寫入都有外部還原路徑，但「還原得回來」
與「知道要還原什麼」是兩件事。App 一次操作可能改動多個檔案的多條邊；沒有記錄時，使用者
面對 `git status` 的一片修改，無法分辨哪些是自己改的、哪些是 App 改的。還原路徑存在而
不可用，等於不存在。

**Consequence**：無記錄時使用者的實際選擇會是「全部還原」或「都不還原」，前者連帶丟棄
自己的編輯，後者留下不想要的變更。

**Action**：該記錄**只存在於 session 內的畫面**，不落地為檔案——落地即違反本節總原則。
記錄的呈現形態落 `0.1.0-W3-073`。

### 四、PII：寫入路徑的可觀測性只記識別，不記內容

**結論**：與寫入路徑相關的一切記錄（錯誤攔截、失敗訊息、未來若加入的執行期 log、第三項
的變更記錄）一律只記**識別**——檔案路徑、欄位名、寫入結果、失敗原因分類；**禁止記錄
欄位值與檔案內容**。

**Why 原重評條件不足**：原文的重評條件只綁「日後加入 log」。但新的 PII 面不來自 log 這個
機制，來自「App 開始碰使用者內容」這件事——不寫 log 也存在，因為全域錯誤攔截三層
（`0.1.0-W1-016`）會捕捉例外，而例外訊息若嵌入了正在寫入的內容，內容就經由錯誤路徑外洩，
且該路徑通常沒有人在設計時把它當成資料出口看。

**Consequence**：使用者的文件可能含敏感資訊。內容一旦進入例外訊息，就會出現在崩潰報告、
終端輸出、使用者為了求助而貼出的截圖裡——這些出口都在 App 的控制之外。

**Action**：本條為 `0.1.0-W3-073` 規格的強制條文，並綁 `0.1.0-W1-016` 的錯誤攔截實作：
攔截層對寫入路徑的例外須在記錄前剝除內容欄位。git 本身存有內容不構成新的 PII 面——那是
使用者自己的 repo，不是 App 新增的持有者。

### 五、授權檢查：唯一判準是寫入路徑必須位於使用者選定的工作區之內

**結論**：物件層級授權檢查仍不適用（單使用者、本機，無多主體），但其位置由**寫入範圍
界限**承接：每一次寫入前，將目標路徑正規化（解析 symlink 與 `..`）後驗證其位於使用者
選定的工作區之內，超出即拒絕寫入。

**Why 這是跨線後「授權」的實際形態**：本專案的 App Sandbox 已關閉（PROP-001，因沙盒下
無法執行 doc CLI 亦無法讀取任意資料夾）。沙盒關閉意味著 OS 層對本 App 的寫入範圍沒有任何
邊界，App 自己的檢查是唯一的邊界。原「單使用者、本機、唯讀」三個理由中，唯讀已不成立，
而前兩者從來就不是範圍的保證——它們回答的是「誰可以存取」，不是「可以存取到哪裡」。

**Consequence**：目標路徑並非全由 App 產生——它來自被掃描文件的欄位（現況已有先例：
破洞報告以文件中的 `filePath` 欄位對外開啟檔案）。文件內容是使用者可編輯的輸入，
含 `../` 的路徑欄位會使寫入落在工作區之外，而使用者不會預期選定一個工作區之後 App 會動
到別處的檔案。

**Action**：本條為 `0.1.0-W3-073` 規格的強制條文。既有的對外開啟路徑（`gap_report_screen`
的開啟原始檔）屬讀取側同型風險，其處置不在本票範圍，由 `0.1.0-W1-068`（ExternalOpener
抽象）承接時一併考慮。

### 五項皆為結論，無延後項

`decision-trigger-binding` 規則 1 要求每項為「已決策」或「綁 ticket ID 的延後」。本節五項
全部落在前者。各項 Action 中指向 `0.1.0-W3-073` 者為**落地載體**而非延後——結論已定，
`0.1.0-W3-073` 承接的是把結論寫成規格條文，不是重新決定結論。

**tripwire 狀態**：「加入編輯能力」一條的規定 action 至此執行完畢。該條的偵測承擔者欄
（`0.1.0-W3-062` 所補）指向本票，本票完成即該條結案。
