...
- FR-06 查詢不可用時，不產生 `parseFailure` 破洞，破洞報告顯示「無法判定破洞」並說明原因。原因以原因碼表示（專案型別表版本高於內建版本／型別表沒有路徑模式），屬資料值；顯示文字由畫面經 l10n 投影（SPEC-004 v1.46 已有對應 key），Diagnostics 不產生在地化字串（用戶裁決 2026-09-24）
