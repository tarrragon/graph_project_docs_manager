...
**版本**: 1.24（第 1-3 章已核定；第 4-5 章逐元件契約與容器不變式由 `0.1.0-W1-044.2` 填寫；第 6-7 章依 §3.7 第 7 項填最小集，標提案；對比核定依 §3.7 第 25 項回填；1.17 起 §1 回饋通道子表與 §2 過渡提示觸發點／樂觀更新政策兩列為提案；逐元件回饋契約與狀態矩陣來源／同步策略欄由 `0.1.0-W3-060.2`–`060.5` 填寫，1.22 起全部 42 條目落成，§4.0.10 定位提示容器執行約定為提案，1.23 補其雙向接線與場景分支，1.24 統一正文用語一致性（zhtw strict 兩項，見變更歷史）並移除既有導覽頁計數慣例的表面數字）
**來源**: PROP-004
...
> §1〈回饋通道子表〉locate 列把「高亮 token 與時長」交由第 4 章對應容器條目定義，SPEC-003 §2.2「系統層通知」點擊導向列引用同一列；4.18／4.21／4.23 的 locate 列亦以「由容器條目補」路由至此。本節定一次作法，容器條目的 locate 列只寫「依 §4.0.10」與自己的場景、目標與分工，不重述機制。

| 項目 | 值 |
|------|-----|
| 執行端 | 持有目標所在 `ScrollController` 的容器：一維捲動為 4.30 `Panel.scrollable`（`scroll-gaps-sections` / `scroll-traceability-tree` / `scroll-tickets-topics` 錨點的持有者）或 4.36 `DataTable`；二維為 4.37 `MatrixGrid` / 4.38 `SwimlaneGrid`。被定位的子件（4.40 `ListRow.item` 等）只依傳值渲染高亮，不捲動、不持有定位狀態（§2 頁面狀態保留） |
...
| 高亮時長 | 機制非數值：自焦點移入被定位項起，至焦點離開該項或任一指標按下為止；不新設 `Motion` token（SPEC-003 §2.1 無對應時限；若 PM 要求數值化時長，由 UX 審查補 token 後改本列）；§1 子表與 SPEC-003 §2.2 的「短暫」於 0.1 即指此焦點綁定機制 |
...
| 定位提示 locate | 系統 | 由容器承載（4.39 `Tree` / 4.41 `Section`）：§1 子表 locate 列適用場景「樹節點展開後的子項」的執行端是承載子層的容器——展開後新出現的子項若落在 viewport 外，scroll-into-view 與短暫高亮的 token 與時長由該容器條目定義，機制依 §4.0.10（場景 c：最小捲動、不高亮）（SPEC-003 §3.3 只約束 offset 不歸零，未定義子項定位，提案：由容器條目補）；本元件只回呼 `onToggle`，不參與定位；本元件自身不是被定位目標 | — | — | 由容器承載（§1 子表「定位提示」列） | 不適用 | 規格撰寫者 |
...
| 定位提示 locate | 系統 | 一處場景：系統通知點擊導向且結果為 `state-gaps-none` 時，焦點移入本元件根節點、不高亮（SPEC-003 §2.2「點擊通知的導向」列）；焦點移入由頁面層（`ScanNotifier.activated` 消費者）執行，機制依 §4.0.10（觸發者與定位失敗列），本元件根節點須可接受焦點（`Focus` 節點，`skipTraversal: true` 不佔 Tab 順序；提案，SPEC-003 只定焦點落點未定載體）。其餘空狀態 0.1 無定位場景（由 4.28 `PageColumn` / 4.30 `Panel` 承載） | 無（不高亮） | 無（SPEC-003 §2.2 未設焦點移入時限；提案） | §1 子表「定位提示」列（輔助技術：焦點移入被定位項；視覺高亮不適用） | 不適用 | 反應：規格撰寫者（提案；SPEC-003 §2.2 定落點）；通道：規格撰寫者 |
...
| 定位提示 locate | 系統 | 本容器承載跨畫面定位的**切頁**段：系統通知點擊／`viewGapsAction` 時將 `selectedDestinationProvider` 設為 `AppDestination.gaps`、`returnToProvider` 設為 `null`（SPEC-003 §2.2「點擊通知的導向」列、§2.3 規則 1）；頁內 scroll-into-view 與高亮不由本容器執行，依 §4.0.10 由 4.30 `Panel.scrollable`（`scroll-gaps-sections`）承載；本容器自身（側欄、標題列）常駐不捲動，不是被定位目標；overlayOpen 時通知點擊視同點外部（4.42 退出路徑）：先收合浮層（`switcherOpenProvider` 改 `false`，焦點依焦點列回 `project-switcher-entry`）再執行切頁段與定位段，使焦點移入被定位項不與浮層 `FocusScope` 衝突（提案） | 無（`IndexedStack` 切頁無轉場，SPEC-003 §2.8） | 無（SPEC-003 §2.2 未設切頁時限；提案） | §1 子表「定位提示」列（切頁段由本容器承載，定位段依 §4.0.10） | overlayOpen → default（通知點擊先收合浮層，提案）；default 下切頁不改變本狀態 | 反應：UX 審查（SPEC-003 §2.2）；分工：規格撰寫者 |
...
| 定位提示 locate | 系統 | 不適用（無承載場景）：本容器為覆蓋層，每次展開自頂端、offset 不保留（SPEC-003 §3.7 捲動列），無定位至專案項的場景；目前專案項以 `selected` 態（4.9）標示，非定位；本容器不是被定位目標（§1 子表 locate 列適用場景皆在導覽頁內） | — | — | 不適用（覆蓋層無定位場景） | 不適用 | 規格撰寫者 |
