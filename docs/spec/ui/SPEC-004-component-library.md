...
| 版型策略 | **單一響應式元件庫**（用戶簽核 2026-09-02） | 依方法論〈形態因素先決〉版型拆分約等於元件庫雙軌的長期維護成本，本專案單平台單視窗，無拆分理由 |
...
PM 核定 §3.3 二十項與 §1、§3.5、§3.6 的待核定項；標「用戶簽核」者由專案擁有者裁示。第 4-5 章依本表填寫；§3.1 總表中與本表不一致處以本表為準，由 044.2 回填總表。

| # | 項目 | 核定 | 理由 |
|---|------|------|------|
| 1 | §1 版型策略 | 單一響應式（用戶簽核） | 單平台單視窗，無拆分理由 |
