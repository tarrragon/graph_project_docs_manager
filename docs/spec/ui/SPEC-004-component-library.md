...
| 等待與結果 | 系統 | 不承載三層中任一層：點擊確認由子件承載；等待指示由各頁內容 slot 的 4.24 `LoadingState` 承載（SPEC-003 §2.6 畫面級載入態）；結果通知為切頁／浮層開合的狀態轉換本身（SPEC-003 §2.2 結果通知列）；SnackBar 由 4.26 經 `ScaffoldMessenger` 承載，本容器提供 `Scaffold` 宿主（提案，沿用既有 `shell.dart`） | — | — | 不適用（三層皆由子件或內容 slot 承載；`Scaffold` 宿主非回饋通道） | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載：等待指示由內容 slot 的 4.24 `LoadingState` 承載（SPEC-003 §2.6），結果通知為換件本身（SPEC-003 §2.2 結果通知列），點擊確認由子件承載 | — | — | 不適用（三層皆由內容 slot 子件承載） | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載：三層皆由子件或內容 slot 元件承載（SPEC-003 §2.2） | — | — | 不適用 | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載：右欄與主欄換件的資料為本地已解析內容、無等待指示（SPEC-003 §3.1「不顯示任何等待指示」、§3.6）；結果通知為換件本身（SPEC-003 §2.2 結果通知列） | — | — | 不適用 | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 本列承載三層中的**點擊確認**層（整列 pressed 態）；結果通知為 `IndexedStack` 可見頁更換的狀態轉換本身（SPEC-003 §2.2 結果通知列）；無等待指示（jump 無載入，節點詳情資料為本地已解析內容，SPEC-003 §2.6） | Material 內建 | `Motion.feedback` | §1 子表「等待與結果」列 | 不適用 | 反應：UX 審查（SPEC-003 §2.2、§2.6）；通道：規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載等待指示（Ticket 載入由 4.24 `LoadingState` 承載，本容器於載入完成後才渲染，SPEC-003 §2.6、§3.4）；結果通知為列集合改變的狀態轉換本身（SPEC-003 §2.2 結果通知列：搜尋／篩選／排序後筆數與首末列改變）；點擊確認由 4.35 / 4.14 承載 | — | — | 不適用（三層由 4.24 / 4.35 / 4.14 與列集合轉換承載） | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 列首承載三層中的**點擊確認**層（pressed 態）；結果通知為列高亮與右欄內容的狀態轉換本身（SPEC-003 §2.2 結果通知列）；無等待指示（選取為本地狀態，右欄資料為本地已解析內容，SPEC-003 §3.1「不顯示任何等待指示」）；載入期間由 4.24 承載，本容器於 `state-domain-matrix` 才渲染 | Material 內建 | `Motion.feedback` | §1 子表「等待與結果」列 | 不適用 | 反應：UX 審查（SPEC-003 §2.2、§3.1）；通道：規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載：無可點元素故無點擊確認層；無等待指示（泳道為假資料靜態排版，切換模式為 `state-domain-matrix` ↔ `state-domain-swimlane` 的 cross-fade 由畫面承載，SPEC-003 §3.1）；結果通知為模式切換與定位本身 | — | — | 不適用 | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載等待指示（無載入態，SPEC-003 §3.3 首次可見；展開為本地重建，SPEC-003 §2.6「0.1 無非狀態級短暫等待」）；結果通知為子層出現／消失的狀態轉換本身（SPEC-003 §2.2 結果通知列）；點擊確認由 4.18 / 4.40 / 4.6 承載 | — | — | 不適用 | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 本列承載三層中的**點擊確認**層（整列 pressed 態）；結果通知：`tree` 為 `IndexedStack` 可見頁更換的狀態轉換本身、`item` 為 SnackBar（狀態不變的動作，SPEC-003 §2.2 結果通知列與外部開啟契約，由 4.26 承載）；無等待指示（jump 無載入；外部開啟實測 600 ms 內且不顯示等待，SPEC-003 §2.2 外部開啟契約） | Material 內建 | `Motion.feedback`；`item` 的 SnackBar 停留 `Motion.snackBar` / `Motion.snackBarWithAction`（由 4.26 承載） | §1 子表「等待與結果」列（`item` 結果通知＝SnackBar） | 不適用 | 反應／時限：UX 審查（SPEC-003 §2.2、§3.3、§3.5）；通道：規格撰寫者 |
...
| 等待與結果 | 系統 | 不承載等待指示（項目資料已載入，展開為本地重建，SPEC-003 §2.6「0.1 無非狀態級短暫等待」）；結果通知為項目出現／消失的狀態轉換本身（SPEC-003 §2.2 結果通知列）；點擊確認由 4.18 與項目條目承載 | — | — | 不適用 | 不適用 | 規格撰寫者 |
...
| 等待與結果 | 系統 | 點擊確認由 4.9 / 4.4 承載；無等待指示：清單為本地 fixture／已探測結果，探測逾時期間浮層仍可操作、其餘項不受影響（SPEC-003 §3.7 探測逾時的處理）；系統資料夾選擇器開啟期間亦無等待指示（選擇器為系統 UI）；結果通知為收合 + Domain 視圖轉入載入態的狀態轉換本身（SPEC-003 §2.2 結果通知列、§3.7），載入態由 4.24 承載 | `Motion.overlay`（收合） | 收合於 `Motion.overlay` 內；載入態的等待由 4.24 承載 | §1 子表「等待與結果」列（結果通知＝收合 + 載入態轉換） | 不適用 | 反應：UX 審查（SPEC-003 §2.2、§3.7）；通道：規格撰寫者 |
