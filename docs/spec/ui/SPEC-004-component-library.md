...
| 高亮 token | 目標子件底色 `AppColors.surfaceIconTint`（與 4.37 列高亮、4.9 目前專案項同一 token，語意為「被指出的那一列」），圓角同子件既有 hover 區；文字色不變（`textPrimary` / `surfaceIconTint` 對比已於 §4.0.2 驗證）；無入場動畫（離散出現，與 SPEC-003 §3.1「格選中態出現」同理） |
...
| 狀態變更提示 state-change | 呼叫端（本容器為 `ConsumerWidget`，於 `ref.listen` 依選取 provider 改列高亮與向 4.15 傳值；含 Esc、`action-domain-cell-clear`、切換專案（選取清除，SPEC-003 §3.1 生命週期）等非列首點選引起的改變） | 列高亮（`surfaceIconTint`）的出現、移動、消失為離散替換，無入場動畫（持續性標記，同 SPEC-003 §3.1「格選中態出現」）；列首 `selected` 旗標改變由輔助技術播報（無障礙子節，原生旗標不另組字）；首次渲染不做逐格入場動畫（SPEC-003 §3.1 動畫提示） | 無（SPEC-003 §3.1 明訂無入場動畫；右欄 `Motion.transition` 由 4.31 承載） | `Motion.feedback`（選格或選列引起者，SPEC-003 §3.1 選格列）；清除與切換專案引起的 default 回復無時限條文（本地即時；提案） | §1 子表「狀態變更提示」列（`Motion.*` 過渡不適用；無障礙播報適用（列首 `selected` 旗標）；不升級系統通知） | default → rowSelected（列首或格被選）/ rowSelected → rowSelected（高亮移動）/ rowSelected → default（切換專案） | 反應／時限：UX 審查（SPEC-003 §3.1）；通道／狀態邊：規格撰寫者 |
...
| 焦點 focus | 使用者／輔助技術 | 展開時焦點移入浮層首個可聚焦項（專案項或選擇其他）並唸出 `switcherTitle`（`Semantics(scopesRoute: true)`，無障礙子節）；焦點限制於浮層（`FocusScope` + `FocusTraversalGroup`）、Tab 依序項目 × N → 選擇其他、不離開浮層（SPEC-003 §2.10「浮層展開時」列）；Esc 收合、焦點回到 `project-switcher-entry`（SPEC-003 §2.10、§3.7 按 Esc 列）；各停留點焦點環由 4.9 / 4.4 依 §4.0.1 承載 | 無 | 無（SPEC-003 §2.10 只斷言限制與落點，不設時限；提案） | §1 子表「焦點」列（焦點限制與 Esc 由本容器承載；焦點環由子件承載） | expanded / noRecent → Esc →（本容器消失，入口回 collapsed，4.8） | 反應：UX 審查（SPEC-003 §2.10、§3.7）；通道／狀態邊：規格撰寫者 |
