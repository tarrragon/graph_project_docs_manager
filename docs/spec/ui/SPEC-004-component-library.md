...
**SPEC-001 狀態列顯示欄 → 本檔**（狀態元件內部的訊息／說明／動作皆為該元件的 slot，不另列；本表列數與 SPEC-001〈狀態總數〉的加總值相同、順序依 SPEC-001 各節，數值以該節為準不在此複述；本版對齊 SPEC-001 v1.9）：
