...
| 等待與結果 | 系統 | 本元件承載三層中的**點擊確認**層（pressed 態）；等待指示為畫面級（4.24 `LoadingState`，SPEC-003 §2.6）、結果通知為狀態轉換本身或 SnackBar（4.26），皆不由本元件承載；無 loading 狀態 | Material 內建 | `Motion.feedback` | §1 子表「等待與結果」列 | 不適用 | 反應：UX 審查（SPEC-003 §2.2）；通道：規格撰寫者 |
...
| 等待與結果 | 系統 | 本元件承載三層中的**點擊確認**層（pressed 態）；結果通知為 jump 至 `nav-page-gaps` 的狀態轉換本身（SPEC-003 §2.2 結果通知列），無等待指示（本地已解析內容） | Material 內建 | `Motion.feedback` | §1 子表「等待與結果」列 | 不適用 | 反應：UX 審查（SPEC-003 §2.2）；通道：規格撰寫者 |
...
| 等待與結果 | 系統 | 本元件承載三層中的**點擊確認**層（pressed 態）；結果通知為 rail 切換的狀態轉換本身（可見頁更換 + 本項轉 selected，SPEC-003 §2.2 結果通知列）；無等待指示（`IndexedStack` 全部導覽頁已建構，SPEC-003 §2.8） | Material 內建 | `Motion.feedback` | §1 子表「等待與結果」列 | 不適用 | 反應：UX 審查（SPEC-003 §2.2、§2.8）；通道：規格撰寫者 |
