...
**版本**: 1.22（第 1-3 章已核定；第 4-5 章逐元件契約與容器不變式由 `0.1.0-W1-044.2` 填寫；第 6-7 章依 §3.7 第 7 項填最小集，標提案；對比核定依 §3.7 第 25 項回填；1.17 起 §1 回饋通道子表與 §2 過渡提示觸發點／樂觀更新政策兩列為提案；逐元件回饋契約與狀態矩陣來源／同步策略欄由 `0.1.0-W3-060.2`–`060.5` 填寫，1.22 起全部 42 條目落成，§4.0.10 定位提示容器執行約定為提案）
**來源**: PROP-004
...
| 項目 | 值 |
|------|-----|
| 執行端 | 持有目標所在 `ScrollController` 的容器：一維捲動為 4.30 `Panel.scrollable`（`scroll-gaps-sections` / `scroll-traceability-tree` / `scroll-tickets-topics` 三個錨點的持有者）或 4.36 `DataTable`；二維為 4.37 `MatrixGrid` / 4.38 `SwimlaneGrid`。被定位的子件（4.40 `ListRow.item` 等）只依傳值渲染高亮，不捲動、不持有定位狀態（§2 頁面狀態保留） |
...
| 高亮時長 | 機制非數值：自焦點移入被定位項起，至焦點離開該項或任一指標按下為止；不新設 `Motion` token（SPEC-003 §2.1 無對應時限；若 PM 要求數值化時長，由 UX 審查補 token 後改本列） |
...
| 定位提示 locate | 系統 | 由容器承載（4.39 `Tree` / 4.41 `Section`）：§1 子表 locate 列適用場景「樹節點展開後的子項」的執行端是承載子層的容器——展開後新出現的子項若落在 viewport 外，scroll-into-view 與短暫高亮的 token 與時長由該容器條目定義（SPEC-003 §3.3 只約束 offset 不歸零，未定義子項定位，提案：由容器條目補）；本元件只回呼 `onToggle`，不參與定位；本元件自身不是被定位目標 | — | — | 由容器承載（§1 子表「定位提示」列） | 不適用 | 規格撰寫者 |
...
| 定位提示 locate | 系統 | 一處場景：系統通知點擊導向且結果為 `state-gaps-none` 時，焦點移入本元件根節點、不高亮（SPEC-003 §2.2「點擊通知的導向」列）；焦點移入由頁面層（`ScanNotifier.activated` 消費者）執行，本元件根節點須可接受焦點（`Focus` 節點，`skipTraversal: true` 不佔 Tab 順序；提案，SPEC-003 只定焦點落點未定載體）。其餘空狀態 0.1 無定位場景（由 4.28 `PageColumn` / 4.30 `Panel` 承載） | 無（不高亮） | 無（SPEC-003 §2.2 未設焦點移入時限；提案） | §1 子表「定位提示」列（輔助技術：焦點移入被定位項；視覺高亮不適用） | 不適用 | 反應：規格撰寫者（提案；SPEC-003 §2.2 定落點）；通道：規格撰寫者 |
...
| 定位提示 locate | 系統 | 由容器承載（4.28 `PageColumn`）：0.1 無定位至本元件的場景——阻擋狀態為全頁內容；面板展開後若超出 viewport 由 `PageColumn` 的捲動承載，不對面板執行 scroll-into-view（SPEC-003 §3.1 未定義；提案） | — | — | 由容器承載（§1 子表「定位提示」列） | 不適用 | 規格撰寫者 |
...
| dismissed | 不存在 | 無 | 上述退出 | 不適用：靜止（下一次 `show` 重新進入） | —（同上；不存在即為預設） | 本地即時 |
...
| `locateTarget` | `Key?`（`scrollable`；§4.0.10 定位入口：值自 `null` 變為某子件 key 的那次 build，本容器於 post-frame 對該 key 執行捲動與焦點移入；頁面層於高亮結束時改回 `null`，同值不重複定位） | 否（提案） | 不適用 |
...
| 狀態變更提示 state-change | 呼叫端（本容器為 `ConsumerWidget`，於 `ref.listen` 依 `expandedTraceNodesProvider` 重建可見列集合並播子層過渡、依 `traceabilityStateProvider` 重建節點樹；含切換專案時展開集合清空、offset 歸零（SPEC-003 §3.3 生命週期）） | 子層列出現或消失的高度變化 `Motion.transition`，`disableAnimations` 時瞬間展開（SPEC-003 §3.3 動畫提示）；子層出現由 4.18 `expanded` 旗標播報（無障礙子節）；缺口虛線框靜態、不閃爍（SPEC-003 §3.3 動畫提示）；三個畫面狀態之間的 cross-fade 由 4.28 承載 | `Motion.transition`（子層高度，本容器承載） | 無（SPEC-003 §3.3 動畫提示只定形式不設時限；提案） | §1 子表「狀態變更提示」列（`Motion.*` 過渡適用（本容器）；無障礙播報由 4.18 承載；不升級系統通知） | 不適用（本容器單一狀態；展開集合為資料態的值變化） | 反應：UX 審查（SPEC-003 §3.3）；通道：規格撰寫者 |
