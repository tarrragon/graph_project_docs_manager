...
> L3 專案元件庫章節（本檔）。本檔為 L3，承載元件清單、逐元件契約（九欄位）、容器元件與排列不變式、
...
>
> **本節狀態：提案，待核定。** SPEC-001 / SPEC-002 皆無形態因素明文；下表依 CLAUDE.md §6
> 的視窗尺寸決策與 PROP-004〈版型定案〉填寫，理由欄註明來源。簽核未回前，
> 第 4 章尺寸契約與測試點的「每種尺寸」列標「待決」，其餘欄照填。
...
| 測試尺寸集合 | 最少兩種：`kMinWindowSize`、`kDesignSize`；是否加入第三種「大於設計基準」尺寸（驗證填滿父容器的元件不拉變形）**待核定** | 兩種尺寸皆有具名常數可引用；第三種尺寸無專案決策來源，不自行發明數值 |
...
| 文字縮放適應 | **待核定**。提案：0.1 不承諾 a11y 動態字級，`textScaler` 固定；元件高度以字級 token 推算的固定值 | 無任何規格提及動態字級；若承諾則所有固定高度列（表格列、導覽項）須改為彈性高度，影響第 4 章尺寸契約全部條目 |
| 斷點策略 | 單一響應式，無 RWD 斷點；空間不足由各容器的排列不變式（換行／捲動／收合）承載，不以視窗寬度切換版型 | 視窗尺寸下界與設計基準同一數量級，版型差異不足以支撐雙軌；SPEC-001〈設計約束〉「0.1 的斷言是畫得出來、不溢位、能捲能拖」 |
| 版型策略 | **單一響應式元件庫**（提案，待用戶簽核） | 預設值；依方法論〈形態因素先決〉版型拆分約等於元件庫雙軌的長期維護成本，本專案單平台單視窗，無拆分理由 |
...
### 3.3 模式 A 步驟 3：存在必要性檢視（全部為提案，待 PM 核定）
...
| §7 | 無最近專案 | 僅「選擇資料夾…」 | `SwitcherOverlay` 零項 + `AppButton.text`；`EmptyState.compact` 是否渲染訊息**待核定** |

---

## 4. 逐元件規格

> 每個元件依下列結構定義。目標：實作者不需猜測任何行為。九欄位對應的子節（對應表見檔頭註解）缺任一即契約不完整，
