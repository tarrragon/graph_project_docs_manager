...
**三者的日誌現況為三組互斥欄位，本規格不承認其為可消費形態**（處置見
`0.1.0-W3-277`）：輸入驗證失敗發 `rejected` 事件並帶 `reason: emptyId`
而無決策欄；協定違規發 `heldAndRequested` 事件並帶 `reason:
heldAndRequested` 而無決策欄；級別競爭導致的拒絕發 `rejected` 事件並帶
`decision: rejected` 而無 `reason`。以事件名 `rejected` 篩選會同時取得
輸入驗證失敗、且遺漏協定違規；以 `decision` 欄篩選只取得級別競爭一種。
消費端因此無法以單一條件取得「全部拒絕」，也無法區分「仲裁結局」與
「請求不合法」——而後者正是上一段要求區分的那條線。本節第 1 項測試義務
要求每個仲裁決策事件攜帶七項欄位，前兩者缺決策欄即不滿足該要求。
