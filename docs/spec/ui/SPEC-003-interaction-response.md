...
| 欄位 | 本專案的值域 | 依據 |
|------|------------|------|
| 請求識別 | process 內單調遞增整數，於仲裁點受理請求的當下產生（機制比照 `0.1.0-W3-176` `showId` 的產生方式：`static int` 計數器，無 `await` 介入無競態），與 `AppSnackBarLogSink` 的 `showId` 屬不同計數空間，關聯要求見上節 | `0.1.0-W3-176` Phase 1 設計決策 D1 的既有先例 |
...
| 決策（離開方式） | 五值之一：立即呈現於原判定載體／升級呈現（§2.13 T1 判否承接 (a)）／延後呈現（承接 (b2)）／狀態承載不另發（承接 (b1)）／不顯示且留痕（承接 (c)） | §2.13 判準 T 與 T1 判否三承接的既有結論；不重新推導，僅將既有四類結局收斂為決策欄的值域 |
...
被否決）。**「拒絕」「合併」「降級」三值本專案不使用**：「拒絕」是
等待型在通道飽和時的動作，本專案的等待型請求（§2.14 列 5–10）皆為
使用者當下操作觸發，觸發當下即代表使用者正在看著畫面，T1 恆判是，不
落入 (a)/(b1)/(b2)/(c) 的「拒絕」情境，且 §2.16 已證本通道無佇列滯留，
沒有「因通道已滿而拒絕」的成立條件；「合併」需三條件 AND（同一來源、
