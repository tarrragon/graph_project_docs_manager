...
則，重疊在本專案不可能發生；`AppSnackBar.show` 在顯示前無條件 `hideCurrentSnackBar()`，
使前一則永遠等不到自己的停留時間。此行為為 §4.26 狀態矩陣退出路徑「新的 SnackBar
...
`AppSnackBar.show()` 載體（`lib/components/app_snack_bar.dart:121`），
兩者的到達時間窗可能重疊（如使用者在視窗非前景時開啟外部檔案，
恰於視窗回到前景、列 3 的延後 SnackBar 也要顯示的時刻）。
`AppSnackBar.show()` 目前的實作是無條件 `hideCurrentSnackBar()`
後顯示新的一則——不讀取目前佔用者的級別或來源，是「後到者無條件
取代」，不是規則 2 要求的排隊或合併。
...
**驗收（現況記錄，非「通過」）**：`AppSnackBar.show()` 原始碼中
`hideCurrentSnackBar()` 呼叫（`app_snack_bar.dart:121`）不含任何
比較目前佔用者級別的邏輯——此即規則 2 未被滿足的靜態證據；成立
...
| 0.1 本通道連「進入卸載狀態」都不存在 | §2.13〈顯示位爭用時的約束與 0.1 的處置規則〉：「現況形態是替換截斷，不是視覺重疊」，SnackBar 佇列深度恆為 0 或 1（無條件 `hideCurrentSnackBar()` 後立即顯示新一則），沒有「佇列滿到某個閾值才開始卸載」這個狀態機分支可言——卸載表（〈讓步與卸載順序〉九格表）在此通道現行實作下不由佔用量驅動任何決策 |
...
| 佔用量快照 | 現行架構下恆為 0 或 1 | §2.16〈佔用量觸發型暫停（滯後）在注意力通道的形態〉已證：`AppSnackBar.show()` 無條件 `hideCurrentSnackBar()` 後立即顯示，佇列深度架構上無中間值，本節沿用該結論不重新推導 |
