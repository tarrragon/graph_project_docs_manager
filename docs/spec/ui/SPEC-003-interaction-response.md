...
| 覆蓋狀態 | 空圖、無 UC、無提案、無 ticket、無破洞 | 不是框架專案、無可消費的型別表、schema 不相容 |
| 必備動作 | 至少一個非「返回」的前進動作（SPEC-001 FR-03） | 至少一個出口（切換專案），且該出口恆可用 |
| 動作觸發後 | 內容跳轉（jump，設 `returnTo`）或同畫面轉換（選擇資料夾、重新掃描）（`0.1.0-W3-335.37` R10） | 開啟專案切換浮層，不改變 `IndexedStack` 索引 |
...
| T2 | 事件是否在**使用者當前可見的畫面上**產生可見的狀態轉換？ | 是否有對應的 `state-*` 錨點變更，且該錨點在可見頁上 | **是 → 不用暫態提示**（狀態轉換本身即結果，§2.2 三層表「結果通知」列第一分支） |
...
| (b) 延後至該畫面再度可見 | 兩種形態：**(b1) 狀態承載**——結果為持久狀態時由狀態本身滿足，使用者回到該畫面即見，不另發提示；**(b2) 延後顯示**——提示照樣顯示，只是推遲到視窗回到 `resumed` 的那一刻 | (b1)：Domain 載入完成、Ticket 載入完成（§2.2〈適用範圍〉不升級的理由即此）。(b2)：`denied` 分支的掃描完成 SnackBar（§2.2 權限 gate 表「顯示時機」，且回到前景前使用者已自行進入 `nav-page-gaps` 時不顯示） |
...
| 5 | `scanCompleteNoGapsSnackbarMessage`（權限 `denied` fallback） | 是（顯示時機已由 §2.2 權限 gate 表限定為前景且非 gaps 頁） | 否（轉換在不可見的 gaps 頁） | 否 | 是（`viewGapsAction`） | `withAction` | §2.2：`withAction` | 是 |
| 6 | `scanCompleteSnackbarMessage(count)`（同上，有破洞） | 同上 | 同上 | 否 | 是（`viewGapsAction`） | `withAction` | §2.2：`withAction` | 是 |
