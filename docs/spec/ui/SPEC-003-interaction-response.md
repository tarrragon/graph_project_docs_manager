...
| 版本 | 日期 | 變更 |
|------|------|------|
| 1.25 | 2026-09-14 | 上游缺描述回寫（`0.1.0-W3-335.13`，對應 SPEC-001 v1.7）：§3.4 新增子節〈篩選與排序的 key 值域〉，列舉 `action-tickets-filter-<key>` 與 `action-tickets-sort-<key>` 的取值並標出處；§3.4〈未載入態不顯示預估耗時〉段註記 SPEC-001 已回寫；§3.1 檢視 schema 詳情列補面板內容恰為兩列版本值、無其他說明或動作；§3.6 重新整理三分支補「`Motion.cancelDeadline` 是品質保證而非逾時觸發條件，無逾時結果」。不改任何既有錨點與狀態 |
