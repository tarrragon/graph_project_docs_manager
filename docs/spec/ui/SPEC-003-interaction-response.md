...
本規格的每一條敘述都寫成「**明確條件 + 可觀察結果**」，使整合測試可直接引用而
不需再次判讀。凡是無法指出斷言對象的措辭（適當、流暢、合理、良好體驗）皆不出現。
可觀察結果只取三種形式：

| 形式 | 範例 |
|------|------|
| 錨點存在性 | `find.byKey(Key('state-domain-unset'))` 為 `findsOneWidget` |
| 錨點屬性 | 該按鈕的 `enabled` 為 `false`；該文字為 `l10n.cancelLoadingAction` |
| 幾何關係 | 該泳道列的 rect 與 viewport rect 有交集 |
...
| C4 | 按下取消後至抵達目標態之間 | 畫面維持載入態版面（骨架與版位不變）；進度指示改為 indeterminate；計數文字凍結於最後值；不得閃現空白、不得出現錯誤文字 |
...
| 條件 | 可觀察結果 |
|------|-----------|
| 按 Tab | 焦點依序走過：專案切換入口 → 六個導覽項 → 內容區的可用動作 → 捲動容器 |
...
**選格與切泳道的關係（PM 核定）**：畫布副標「點格子切換至泳道」與右欄詳情卡
不能同時由單擊承擔。四案比較：
