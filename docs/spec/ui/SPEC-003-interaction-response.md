...
| 3 | 卸載順序九格表（到達類別 × 級別） | 九格各自的動作與卸載順序表定一致，每格至少一測 | 3 格適用（自發型×可棄、自發型×不可棄、等待型×可棄）；6 格空成立（推送型三格、須留痕三格） | 3 格適用者：§2.14 列 1–4／11–13／5–10 已有實例，`0.1.0-W3-207` Phase 2 據此逐格核對。6 格空成立者：見下方〈推送型／須留痕空集合的證據鏈〉 |
