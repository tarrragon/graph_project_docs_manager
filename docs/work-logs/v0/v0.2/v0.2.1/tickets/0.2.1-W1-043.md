...
completed_at: '2026-09-24T09:26:51'
created: '2026-09-23'
updated: '2026-09-23'
...
**Completion Time**: 2026-09-24T09:26:51
**Executing Agent**: star-anise-system-designer
**Review Status**: pending
（必填：變更摘要——哪些文件或章節更新、新增或刪除）

變更摘要：SPEC-005 v1.1 -> v1.2。§2.2 回饋點表新增 remove 結果時刻列（clearStoredPath 移除順序、失敗語意、日誌，不觸碰 recentProjects）；新增 §2.4 最近專案清單持久化（key workspace.recentProjects、JSON 陣列 path+lastOpenedAt schema 與範例、唯一性/排序/上限、RecentProject 型別與 loadRecentProjects/addRecentProject 簽章、寫入時機、回饋點承擔者表、損壞處理邊界）；變更歷史補 1.2 列。表格以「契約」標記規格已定未實作。
## Context Bundle
