...
acceptance:
- '[ ] W3-332.3 範圍內 test-assertion-design 缺範例位置補三段鏈式範例'
- '[ ] 補至少一則「元件狀態集與內容政策如何轉為 Given/When/Then 驗證」的範例（W3-332 審查指出測試端無執行期行為範例）'
- '[ ] 範例技術中立且不含本專案識別符'
tdd_phase: null
tdd_stage: []
assigned: false
...
### Task Reference
- 0.1.0-W3-335.9 what: 修補 test-assertion-design 缺引導（G14、G15）

### Rationale Chain
- 0.1.0-W3-335.9 why: W3-332 審查與走查確認 agent 依此文件推導元件與測試時被迫猜測或讀出兩種做法；缺口編號與來源見 W3-332.13 Solution，並以 W3-332 Problem Analysis〈PM 審查彙整草稿〉P1–P9 修正

### Source Acceptance (reference)
- [ref] [ ] 類型 3 命中率與參考等價的二選一判準補齊  # from 0.1.0-W3-335.9
- [ref] [ ] 斷言預期值取自 spec 條文的約束補齊  # from 0.1.0-W3-335.9
- [ref] [ ] 新約束逐條套〈何處需要範例〉判準並記錄  # from 0.1.0-W3-335.9
