...
completed_at: '2026-09-14T12:00:00'
created: '2026-09-14'
updated: '2026-09-14'
...
**Completion Time**: 2026-09-14T12:00:00
**Executing Agent**: thyme-documentation-integrator
**Review Status**: pending
（必填：變更摘要——哪些文件或章節更新、新增或刪除）
**變更摘要**：`.claude/skills/skill-design-guide/references/chained-examples.md` 新增〈語言專屬 skill 的例外：違規／修正程式碼對照〉章節（置於〈完整示範〉與〈與既有範例規格的相容判定〉之間），含三項判定條件表（單一技術定位／查表用途／語言一致，AND 全部成立才適用）+ Why/Consequence/Action。章首章節列表同步補新章節名。

`.claude/skills/skill-design-guide/SKILL.md`：frontmatter `metadata.version` 1.14.0 → 1.15.0；〈按需讀取〉路由表 chained-examples.md 該列涵蓋章節欄補新章節名（雙向齊全）。

`.claude/skills/skill-design-guide/CHANGELOG.md`：新增 1.15.0 條目，記錄變更內容與理由。

**依據**：用戶裁示（記錄於 0.1.0-W3-332.13 Solution 用戶裁決清單 D01，並經父票 Context Bundle 轉載）——語言專屬 skill 的違規／修正程式碼對照不受「禁程式語法」約束，例外條款由本票寫入規格。三項判定條件為本票新增設計，用於滿足 acceptance #2「避免跨技術 skill 援引」，未見於來源票的既有草稿。

**未觸碰**：dart-style-guardian 等下游 skill 本身的違規／修正對照內容——本票僅在 chained-examples.md 建立例外條款，未回頭修改任何引用此例外的下游 skill 檔案（超出本票 where.files 範圍）。
