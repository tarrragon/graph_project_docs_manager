...
acceptance:
- '[x] 檢查清單含 UC 步驟對應檢查，並寫明無 UC 文件時的處置'
- '[x] 明訂選取類前置呈現獨立成列，附判定條件'
- '[x] 附一則三段鏈式範例且不含本專案識別符'
- '[x] 跨畫面共用的選取值須列出設定入口與清除時機的要求已寫入'
tdd_phase: null
tdd_stage: []
assigned: true
creation_accepted: false
started_at: '2026-09-14T12:33:05'
completed_at: '2026-09-14T12:38:35'
created: '2026-09-14'
updated: '2026-09-14'
...
**Completion Time**: 2026-09-14T12:38:35
...
變更摘要：
- `screen-state-matrix.md` 新增〈UC 步驟對應檢查〉（置於〈從 BDD 操作盤點展開〉後）：逐一核對 UC 主要成功場景步驟是否對應矩陣某狀態或可用操作，明訂無 UC 文件時改用 BDD 操作盤點、兩者皆缺時註記「本檢查未執行」不可留白視為通過。
- 新增〈選取類前置呈現：獨立成列與判定條件〉（置於〈容易漏列的狀態：initializing〉後）：三項判定條件（未決定因非等待外部事件／選擇動作由使用者觸發／選定前後呈現或操作有實質差異）皆成立即須獨立成列；補〈跨畫面共用的選取值〉子節（設定入口 + 清除時機）；附一則去識別化三段鏈式範例（清單頁／詳細頁），驗證段依 chained-examples.md 判定屬「設計產出物」，採檢驗問句形態。
- 檢查清單新增對應三項勾選。
- `SKILL.md` 版號 1.7.0 -> 1.8.0，Last Updated 更新；`CHANGELOG.md` 補 1.8.0 條目。
