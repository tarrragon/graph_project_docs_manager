---
id: 0.3.0-W1-045
title: 修復 sync-pull 有衝突時 base SHA 不推進，解衝突後重跑以舊 base 三方合併誤報
type: IMP
status: pending
version: 0.3.0
wave: 1
...
who:
  ...
  history: {}
what: 修復 sync-pull 有衝突時 base SHA 不推進，解衝突後重跑以舊 base 三方合併誤報
when: 框架修復批次
where:
  layer: Infrastructure
  files:
  - .claude/scripts/sync-claude-pull.py
why: '另一專案三輪 sync-pull 實測：解完衝突提交後重跑，因 base 仍為舊值，對「local 已等於上一輪 upstream」的檔案再次報衝突。canonical #88 已觀測（含處置建議）。'
how:
  task_type: Implementation
  strategy: 衝突輪次結束時先記錄 pending base（本輪 upstream SHA），下一輪 pull 若偵測工作區乾淨且 local 檔等於 pending base 版本則以 pending base 為三方基準；或提供 --resolve-done 子命令推進 base。測試以 fixture repo 重現。腳本測試位置依既有慣例。
acceptance:
- '[ ] 解衝突後重跑不對已等於上一輪 upstream 的檔案報衝突（fixture 測試）'
- '[ ] base 推進時機在文件明寫'
tdd_phase: phase1
tdd_stage:
- phase1
...
修復 sync-pull 有衝突時 base SHA 不推進，解衝突後重跑以舊 base 三方合併誤報
