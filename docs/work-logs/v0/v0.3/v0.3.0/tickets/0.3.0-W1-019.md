---
id: 0.3.0-W1-019
title: 實作 最近專案項展開時逐項可用性探測（現一律 enabled）
type: IMP
status: pending
version: 0.3.0
wave: 1
...
who:
  ...
  history: {}
what: 實作 最近專案項展開時逐項可用性探測（現一律 enabled）
when: 0.3.0 開工後
where:
  layer: Presentation
  files:
...
why: SPEC-003 §3.7〈生命週期〉與 SPEC-004 RecentProjectItem disabled 狀態附原因文字：浮層展開時逐項探測資料夾可用性；0.2.1-W1-003 未實作，一律 enabled:true，SPEC-005 §2.4 亦定義不可用項保留於清單、由展開時探測呈現
how:
...
實作 最近專案項展開時逐項可用性探測（現一律 enabled）
