---
id: 0.3.0-W1-018
title: 接上 最近專案項的節點數／票數摘要與健康徽章計數（現為 0/0 佔位）
type: IMP
status: pending
version: 0.3.0
wave: 1
...
spawned_tickets: []
source_ticket: null
discovered_during: 0.2.1-W1-003
scope_blocker: null
dispatch_reason: ''
decision_tree_path:
  entry_point: Layer 1 版本規劃
  final_decision: 0.2.1 凍結且不屬契約 C3 所列條款，建於最近開放後繼 0.3.0
  rationale: C3 只列目前專案名與清單來源；摘要計數與可用性探測為另外的規格條款
who:
  current: parsley-flutter-developer
  history: {}
what: 接上 最近專案項的節點數／票數摘要與健康徽章計數（現為 0/0 佔位）
when: 0.3.0 開工後
where:
  layer: Presentation
  files:
...
why: SPEC-004 RecentProjectItem 定義摘要（節點數 · 票數）與健康徽章 slot；0.2.1-W1-003 移除 fixture 後改顯示 0/0，程式碼註解標為真實 domain 掃描未接線。與 0.3.0 語料解析的節點計數同源
how:
  task_type: Implementation
  strategy: 依規格條款實作並補 widget test；開工前確認與 0.3.0 契約的關係（三問）
acceptance:
- '[ ] 行為符合所列規格條款並有 widget test 斷言'
...
接上 最近專案項的節點數／票數摘要與健康徽章計數（現為 0/0 佔位）
