---
id: 0.3.1-W1-044
title: 修復 version-release finish 啟用下一版本時的提交範圍改為執行前後工作區差集，pubspec 等版本檔不再漏提交
type: IMP
status: pending
version: 0.3.1
wave: 1
...
who:
  ...
  history: {}
what: 修復 version-release finish 啟用下一版本時的提交範圍改為執行前後工作區差集，pubspec 等版本檔不再漏提交
when: 框架修復批次
where:
...
why: '兩個專案各一次 finish 都在「Commit Version Activation」漏掉 ensure_version_activated bump 的 pubspec.yaml，收尾差集守衛 rc=1，各需一張票由代理人在 worktree 補提交。canonical #55 已觀測。'
how:
  task_type: Implementation
  strategy: ensure_version_activated 回傳其觸碰的路徑清單，啟用提交以該清單（或執行前後 git status 差集，與 Step 3 同法）為範圍；PM 不可提交的版本檔由同一常式納入。測試：activation 後工作區乾淨。
acceptance:
- '[ ] finish 完成後工作區無殘留（含 pubspec）測試'
- '[ ] 啟用提交檔案清單與 ensure_version_activated 觸碰清單一致'
- '[ ] 測試全綠，CHANGELOG 版號 +1'
tdd_phase: phase1
tdd_stage:
- phase1
...
修復 version-release finish 啟用下一版本時的提交範圍改為執行前後工作區差集，pubspec 等版本檔不再漏提交
