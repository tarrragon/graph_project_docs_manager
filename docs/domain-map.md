...
- **UC-04 四層樹的第二跳欄位未明訂**：自 PROP 展開時，走
  ...
  候選：盤點 `tracking_schema` 邊表中 PROP/SPEC/UC 間 established 邊與
  frontmatter 欄位，於兩者間擇一或另訂規則。trigger：規劃串真實資料的追溯
  視圖前，於對應 PROP 補記定案，並回填 CLAUDE.md §6 與本節依賴位置
  （SPEC-001 §3、UC-04）（來源票 `0.1.1-W3-369`，已收束為本項知識，未執行）
- **SPEC-001 §4 未載入態「預估耗時」欄無計算依據**：這個詞在規格中出現，
  但沒有任何地方定義它怎麼算，CLAUDE.md §6 五項空殼判準列有此項。SPEC-003
  §2.6 的誠實性硬規則（分母未知時，不得顯示會自行推進的假進度）已使 0.1 只顯示
  票數 N、不顯示預估耗時，但規格層仍宣稱要顯示一個沒有定義的值。候選：
  (A) 以實測樣本建立耗時模型；(B) 判定預估耗時對本 App 無使用價值並自
  SPEC-001 §4 移除該欄——票數本身已足以讓使用者判斷要不要等，顯示一個必然
...
- **跨畫面「首次可見／再次可見」生命週期偵測基礎設施未設計**：SPEC-003 §3.5
  ...
  gap_report／trace 等畫面共用。trigger：規劃處理 SPEC-003 §3.5 再次可見
  自動重掃的 minor 版本時，於對應 PROP 補記定案設計方案（含至少兩個候選：
  擴充 `firstVisibleProvider` 或另建通用模式），並回填各畫面的實作票草案
  （來源票 `0.1.1-W3-384`，已收束為本項知識，未執行）
- **編輯能力四項架構決策與規格前置未完成**：PROP-005 §0.x 已定案「本 App
  直接改檔、CLI 僅作驗證器」的巨觀形態，但落為規格前尚有四項未定案：
  (1) 編輯能力規格本身的實際檔案路徑未指名，且其與既有樂觀更新政策重評的
  承接關係需先校準；(2) tripwire「加入編輯能力」規定重評的寫入前閘門判準
  已知有缺陷（現行以檔案 git 狀態判定，未追蹤檔案落在未定義行為；正確判準
  應改為「本次寫入會失去哪些使用者位元組」，以 HEAD／工作目錄／App 記憶體
  三平面收斂為單一述詞）；(3) W1-013 的 structured log 延後決策三個理由中
  兩個（App 不擁有資料、唯讀）已隨編輯能力定案在決策層失效，僅剩「單機無
  伺服器可收 log」一項待判斷是否單獨撐得住延後；(4) 樂觀更新政策要解鎖，
  需先定義寫入與 CLI 驗證接線的三個可觀測時刻（呼叫發出／外部系統受理／
  結果），依 Port 回饋契約，日誌不可抵扣呼叫端回饋，回滾判斷需要呼叫端能
  可靠區分「寫入成功」與「失敗被降級」。trigger：1.0 提案規劃編輯能力時
  一併定案（來源票 `0.1.1-W3-073`／`0.1.1-W3-280`／`0.1.1-W3-281`／
