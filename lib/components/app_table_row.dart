...
/// 清單——`header` 因欄首元件（`TableColumnHeader`，4.14）尚未建立，改以
/// `Widget` 承接（與 `ListRow` leading/trailing 已記錄的契約差異同一慣例）。
///
...
/// 四變體（SPEC-004 4.35「變體」表）。
enum _TableRowVariant { header, ticket, step, eventFlow }
...
class AppTableRow extends StatelessWidget {
  ...
  /// 事件流小表列。欄序 事件 / 發出 / 消費 / 孤立事件標記，[marker] 可為
  /// `null`（無孤立事件標記）。**非互動列**（無 `onTap`，SPEC-003 §3.2
  /// 〈事件流小表〉無列點擊條文）；可依 [isLocated] 高亮（SPEC-003 §3.5
  /// 事件類破洞項定位）。
  AppTableRow.eventFlow({
    Key? key,
    required AppText event,
    required AppText emittedBy,
    required AppText consumedBy,
    AppIcon? marker,
    bool isLocated = false,
  }) : this._(
         key: key,
         variant: _TableRowVariant.eventFlow,
         columns: eventFlowColumns,
         cells: [
           event,
           emittedBy,
           consumedBy,
  ...
  /// `eventFlow` 欄規格（SPEC-004 4.35「尺寸契約」欄規格 `eventFlow`）：
  /// 事件固定寬（沿用 ID 欄寬）、發出與消費兩填滿欄等分、標記固定寬。
  static const List<ColumnSpec> eventFlowColumns = [
    ColumnSpec.fixed(LayoutSize.ticketIdColumnWidth),
    ColumnSpec.flex(),
    ColumnSpec.flex(),
    ColumnSpec.fixed(LayoutSize.ticketMarkerColumnWidth),
  ...
  bool get _isTappable =>
      _variant == _TableRowVariant.ticket || _variant == _TableRowVariant.step;
  ...
  Widget build(BuildContext context) {
    ...
    final bordered = DecoratedBox(
      decoration: BoxDecoration(
        color: isLocated ? AppColors.surfaceIconTint : null,
        border: Border(bottom: BorderSide(color: _borderColor)),
      ),
      child: row,
    );

    if (_variant == _TableRowVariant.eventFlow) {
      // 非互動列（無 onTap）；located 時焦點移入僅供輔助技術朗讀（SPEC-004
      // 4.35「狀態矩陣」`header` / `eventFlow` 變體無 hover/pressed/focused
      // 說明）。
      return Semantics(
  ...
  String get _semanticLabel {
    final parts = switch (_variant) {
      ...
      _TableRowVariant.eventFlow => [
          (cells[0] as AppText).text,
          (cells[1] as AppText).text,
          (cells[2] as AppText).text,
          if (cells[3] case final AppIcon marker
              when marker.semanticLabel != null)
            marker.semanticLabel!,
        ],
