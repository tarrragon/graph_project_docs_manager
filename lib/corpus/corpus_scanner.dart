...
class _ScanAccumulator {
  ...
  /// 「有 frontmatter 的非節點」（`id` 缺席、不命中、或互斥被打破）。
  void addAvailable(
    TypeTable table,
    String path,
    ...
    if (typing is NodeTypingMatch) {
      rawNodes.add(
        RawNode(path: path, frontmatter: frontmatter, typeName: typing.typeName),
      );
    } else {
      nonNodeCount++;
    }
  ...
  /// 否分流，命中（含平手）者另組裝 EVT-CORPUS-003。
  void addFailure(
    TypeTable table,
    String path,
    ...
    hitCarrierCount++;
    final event = buildParseFailureEvent(path: path, outcome: outcome, table: table);
    if (event != null) {
      parseFailureEvents.add(event);
    }
  }
