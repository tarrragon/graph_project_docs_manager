...
def main() -> int:
    ...
    if not session_id:
        message = (
            "[session-registry-stop-heartbeat-hook] 無法取得 session_id，跳過心跳更新"
    ...
    if registry_paths is None:
        message = (
            "[session-registry-stop-heartbeat-hook] 非 git 環境，跳過心跳更新"
    ...
    try:
        ...
        message = (
            "[session-registry-stop-heartbeat-hook] heartbeat 更新失敗: {}"
            "（本次心跳未寫入，下次 Stop 或 UserPromptSubmit 觸發時會重試，單次失敗通常無需處置）"
        ).format(e)
        sys.stderr.write(message + "\n")
        logger.warning(message)
