...
# ///
"""
Hook Registration Coverage Check

Measures which Python hook files in .claude/hooks/ (and
.claude/skills/*/hooks/) appear as a command in settings.json's `hooks`
config, after removing entries listed in hook-exclude-list.json. This is
a settings.json coverage measurement, not a completeness/correctness
verdict: a hook can be legitimately executed via a channel other than a
direct settings.json entry (e.g. an installer registered in
settings.json that generates a native git hook shim at runtime, or a
helper module imported by another registered hook). Known instances of
such channels are recorded in hook-exclude-list.json's "notes" so they
are not reported here as missing (renamed 2026-09-09 from
"hook-completeness-check.py" — the old name claimed a verdict the scan
never measured).

Runs on SessionStart to surface hook files that do not appear in
settings.json and may need registration (or an exclude-list entry if the
absence is intentional/covered elsewhere).

Exit codes:
    0 - All scanned hooks appear in settings.json (or warning only)
    0 - Hooks not found in settings.json detected (warning, does not block)
"""
...
# ---------------------------------------------------------------------------
# rule8-exempt: relocation:自 hook-completeness-check.py 逐字位置搬移（2026-09-09 改名）
...
def find_merge_declaration_violations(
    ...
    這種語意重複——兩者路徑不同，逐字比對不會命中，卻在同一事件下重複執行
    # rule8-exempt: relocation:自 hook-completeness-check.py 逐字位置搬移（2026-09-09 改名）
    相同業務邏輯（1.4.0-W2-029）。
...
def main():
    logger = setup_hook_logging("hook-registration-coverage-check")
    ...
    # Report results
    log_output = "\n[HookCheck] Hook settings.json 註冊涵蓋率檢查結果"
    ...
    print(log_output)
    logger.info(log_output)
    log_output = f"已出現在 settings.json: {len(registered_hooks)} 個"
    print(log_output)
    logger.info(log_output)
    log_output = f"未出現在 settings.json: {len(unregistered)} 個"
    print(log_output)
    logger.info(log_output)
    log_output = f"排除（見 hook-exclude-list.json，含經其他管道註冊者）: {count_excluded} 個"
    print(log_output)
    logger.info(log_output)
    log_output = (
        f"Skill Hooks (.claude/skills/*/hooks/): 共 {len(all_skill_hooks)} 個"
        f"，已出現在 settings.json {len(registered_skill_hooks)} 個"
        f"，未出現在 settings.json {len(unregistered_skill_hooks)} 個"
    )
    print(log_output)
    logger.info(log_output)
    ...
    if unregistered:
        log_output = (
            "\n未出現在 settings.json 的 Hook 檔（不代表未被執行——見"
            "hook-exclude-list.json 已知替代註冊管道；最多顯示 15 個）:"
        )
        ...
        log_output = (
            "\n建議: 確認這些 Hook 是否需要在 settings.json 中註冊；"
            "若已透過其他管道（installer 產生的原生 git hook shim、被其他"
            "已註冊 hook 匯入的共用模組等）執行，補一筆 hook-exclude-list.json"
            "「notes」說明，而非視為缺漏"
        )
        ...
        log_output = "\n所有掃描到的 Hook 檔案都已出現在 settings.json 中"
    ...
    if unregistered_skill_hooks:
        log_output = (
            "\n未出現在 settings.json 的 Skill Hook（不代表未被執行，"
            "同上；最多顯示 15 個）:"
        )
        print(log_output)
        logger.info(log_output)
...
if __name__ == '__main__':
    sys.exit(run_hook_safely(main, "hook-registration-coverage-check"))
