"""Tests for hook-registration-coverage-check 反向檢查（W9-004 / framework issue #2）.
...
HOOK_FILE = _HOOKS_DIR / "hook-registration-coverage-check.py"


def _load_hook_module():
    spec = importlib.util.spec_from_file_location(
        "hook_registration_coverage_check", HOOK_FILE
    )
    module = importlib.util.module_from_spec(spec)
    sys.modules["hook_registration_coverage_check"] = module
