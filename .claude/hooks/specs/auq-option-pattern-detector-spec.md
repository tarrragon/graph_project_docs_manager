...
**Ticket**: 0.18.0-W5-042 <!-- rule8-exempt: illustration:spec 撰寫出處記述，非引用推導依據 -->
**Phase**: 1（功能規格設計）
...
- `要選哪個`、`哪個比較好`、`請選擇`、`要不要`、`需要做`、`應該`、`先做`、`還是`（各自獨立關鍵字，任一命中即算，非需「先做」與「還是」同時出現的組合片語）
- 最後 50 字內含 `?` 或 `？`
- 最後一行以問號結尾
...
### 3.4-bis Markdown 表格選項偵測（W17-174.2.1 落地） <!-- rule8-exempt: illustration:小節標題記述來源 ticket，非引用推導依據 -->
...
| E2 | 歷史決策回顧（過去時態） | 命中 pattern 前 200 字內須**同時**滿足：(a) 含「先前」「已完成」「過去」「當初」「之前 commit」「歷史」「最後選了」等詞之一；且 (b) 含 ticket 編號（`W\d+-\d+` 樣式的通用 regex，如 W5-042，非字面關鍵字） | 「先前 W5-040 提出了 A.… B.… C.… 三個方案」 <!-- rule8-exempt: illustration:E2 豁免判準的偵測關鍵字子字串與範例文字，非引用推導依據 --> |
| E3 | 程式碼 / 表格註解 | pattern 全部落在 Markdown table cell（`\|…\|` 行）或位於 HTML 註解 `<!-- … -->` 內 | 表格文件化選項 |
| E4 | Hook/規則文件寫作場景 | 訊息含「.claude/」+「.md」路徑字串且佔比 > 10% | 正在編輯規則文件<!-- broken-link-exempt: 字串拼接示意，非路徑引用 --> |
| E5 | 代理人產出內容 | input_data 含 `agent_id`（subagent 環境）→ 直接跳過，不進 pattern 偵測 | 代理人回報內容 |
| E6 | 純資料表格（W17-174.2.1） | 第一個表格的標題列**不含**任何選項關鍵字（選項/方案/策略/Option/推薦/Recommended/建議/候選），僅在「只有」§3.4-bis 表格路徑命中時生效 | 測試結果、效能指標表 <!-- rule8-exempt: illustration:E6 命名括號記述來源 ticket，非引用推導依據 --> |
...
訊息來源：新增常數 `AUQOptionPatternMessages.REMINDER`（.claude/hooks/lib/hook_messages.py <!-- broken-link-exempt: Phase 2 撰寫當時的規劃路徑，此處保留原文為歷史記述 -->）。

實作後共用模組落在 `.claude/lib/hook_messages.py`（頂層，非巢狀於 hooks/ 下）。
...
| TP3 | 二元確認問句（3.3 路徑） | `"Phase 1 剛剛結束。要繼續進 Phase 2 嗎？"` | 命中提醒 |
...
| FP2 | 歷史決策回顧 | `"先前 W5-040 當初提出三個方案：A. 規則強化 B. Hook 補強 C. CLAUDE.md 更新，最後選了 A"` | E2 | 無 additionalContext <!-- rule8-exempt: illustration:FP2 測試範例文字，非引用推導依據 --> |
...
- `.claude/hooks/lib/hook_messages.py` 新增 `AUQOptionPatternMessages.REMINDER` <!-- broken-link-exempt: Phase 2 撰寫當時的規劃路徑，此處保留原文為歷史記述 -->
  （實作後共用模組落在 `.claude/lib/hook_messages.py`，頂層，非巢狀於 hooks/ 下）
...
**Author**: basil-hook-architect (W5-042 Phase 1) <!-- rule8-exempt: illustration:spec 撰寫出處記述，非引用推導依據 -->
