...
**Phase**: 2（測試設計）
**依據規格**: `.claude/hooks/specs/auq-option-pattern-detector-spec.md`（291 行）
**目標實作**: `.claude/hooks/auq-option-pattern-detector-hook.py`（Phase 3b）
**測試檔案**: `.claude/tests/hooks/test_auq_option_pattern_detector.py` <!-- broken-link-exempt: Phase 2 撰寫當時的規劃路徑，此處保留原文為歷史記述 -->

實作後測試檔落在 `.claude/hooks/tests/test_auq_option_pattern_detector.py`（目錄段順序相反）。
...
| TP3 | `test_detects_binary_confirmation_question` | 覆蓋 §3.3 二元問句路徑（無標記但含確認問句+問號） | `"Phase 1 剛剛結束。要繼續進 Phase 2 嗎？"` | 含 reminder additionalContext | AC1, AC6 |
...
| FP2 | `test_exempts_historical_decision_review` | E2 歷史回顧豁免（過去時態 + W5 編號） | `"先前 W5-040 當初提出三個方案：A. 規則強化 B. Hook 補強 C. CLAUDE.md 更新，最後選了 A"` | 無 additionalContext | AC4 <!-- rule8-exempt: illustration:FP2 測試範例文字，非引用推導依據 --> |
...
| `run_hook_safely` wrapper 行為 | 假設沿用 `.claude/hooks/lib/hook_utils.py` 既有實作 <!-- broken-link-exempt: Phase 2 撰寫當時設想的單一檔案，實作後 hook_utils.py 從未以此名成檔，功能拆分落於 .claude/lib/ 下多個 hook_*.py 模組（合併型遷移），此處保留原文為歷史記述 --> | 若 wrapper 介面變更，C3/C4 測試入口可能需調整 |
...
**Author**: sage-test-architect (W5-042 Phase 2) <!-- rule8-exempt: illustration:spec 撰寫出處記述，非引用推導依據 -->
