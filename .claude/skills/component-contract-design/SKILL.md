...
判準的權威在該方法論的〈元件契約判準〉（十一欄位表、容器亦為元件、派發語言）；產物形狀在 `doc` skill 的元件庫規格範本。兩者與其餘外部資產的路徑見 `references/addresses.md`，全文只在該表寫一次。本 skill 承載的是**程序**：從三種不同起點走到契約齊全，各起點的步驟不同，判準相同。
