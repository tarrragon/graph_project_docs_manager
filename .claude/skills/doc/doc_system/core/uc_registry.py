...
# 判定準確性，只是擴大「能被掃到」的範圍。
#
# value 群組後緊接負向前瞻，禁止 value 緊貼的下一段再是另一個
# UC- 前綴（如 select-uc-UC-01 的 select-uc- 片段）：value 字元類
# 含字母數字，若不設此前瞻，遇到 kebab-case 複合識別符中「uc」剛好
# 是前一詞尾、緊接真正的 UC-XX 值時，value 會貪婪吃進後者的字母
# 部分，產出偽 token（如 uc-UC），且搜尋游標越過後真正的 UC-01
# 反而不會被單獨掃到。加上前瞻後，遇到這種情況該候選匹配直接失敗，
# 讓引擎改在後面真正的 UC- 前綴處重新起始匹配。
