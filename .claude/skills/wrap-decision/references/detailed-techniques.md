...
操作：給嚴重性之前寫下評估單位；形態層先量普及度（可 grep 的跑全集合計數，不可的抽樣冷讀）再評級；逐篇審查對可能是模式的發現只標候選，嚴重性由聚合步驟給。兩個評估者分歧時，先問兩邊評的是不是同一個對象。三個反模式：從修復成本推嚴重性（量的是作者側）、新發現直接歸進最接近的既有類別繼承其嚴重性（歸類快於量測是訊號）、評估者自動補完後回報「讀得懂」（審查者做的正是讀者做的事，盲區重合）。這一格在 Step 0 之前，因為對象沒定就擴增選項，擴的是錯的問題。詳見 `principles/evaluation-unit-before-severity.md`。
