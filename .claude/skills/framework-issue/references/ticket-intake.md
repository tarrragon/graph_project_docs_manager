...
| 命中狀況 | 落點 | 動作 |
|---------|------|------|
| 主題已有 open issue、無區段、body 無任何索引表（舊 body-only issue） | 該 issue | `init`（對既有 issue 可執行，追加區段並回填索引）；body 首行手動補 `fw-issue-schema` 標記 |
...
| IMP，pending，未被任何 in_progress 或保留票以 `blockedBy` 依賴 | close；執行內容進「待辦與來源」 |
| IMP，pending，被 in_progress 票或保留票 `blockedBy` 依賴 | 保留 pending，`why` 末尾補一句 issue ref。**Why**：解鎖判定不把 closed 視為已解決，close 它會讓依賴它的執行中工作永遠 blocked |
...
4. 來源票對照（票 ID、型別、標題、處置、落點區段）與分群表的「實際落點」欄以 `ticket track append-log <派發票> --section Solution` 寫入派發票；同機器並行 session 各發一份對照表。
...
步驟：讀票 → 依 references/ticket-intake.md〈步驟三：時序改狀態〉改寫 → sections.json 寫 scratchpad → init／add／observe（依〈步驟二：查重與落點〉）→ 依〈步驟五：ticket 處置〉範圍規則 close → show/check → 對照表以 append-log 寫回派發票 Solution。
禁止：貼入時序敘事、對已有索引的 issue 再 init、close 範圍外或被依賴的票、更新他方 owner 的區段、寫專案內任何檔案。
