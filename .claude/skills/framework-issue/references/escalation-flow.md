...
「修法看似顯然」不構成豁免依據——下方會示範「顯然」本身往往就是未觀測消費端的產物。可在發現當下寫定修法的條件是以下三問皆為「是」：

| 問題 | 檢查內容 |
|------|---------|
| 消費端已觀測 | 修法要改的東西，誰讀它、在什麼條件下執行，是已觀測而非推定？ |
| 反例已試 | 是否做過一個「修法若錯就會失敗」的觀測，而不只是「修法看起來能解掉徵狀」？ |
| 範圍已量 | 涵蓋的呼叫點／檔案／consumer 數是量到的數字，而非估計？ |

**Why**：三問共同排除的是「修法在票面上與已驗證的決定不可分辨」——任一問題留在推定層級，票面的修法敘述外觀不會顯示這個落差，讀者無從得知。
...
**Action**：三問任一為否時，依下表的擴散面分兩檔處置，不是一律升級為分析票：

| 擴散面 | 處置 | 成本 |
|--------|------|------|
| 限本 consumer 且可逆（單檔、不動公開契約） | 同一張票內把修法寫成待驗證形態：標明未驗證項與停手條件，不另開票 | 一個句子 |
| 跨 consumer（`.claude/` 通用資產）、動公開契約、或多呼叫點 | 修法形狀移至 framework issue 區段決定，發現票降為分析票（ANA） | 一張 ANA |

**三個檢驗案例**（判準須能分辨豁免與不豁免，不是只給禁令）：

| 案例 | 三問檢查 | 結論 |
|------|---------|------|
| 測試檔對已棄用 API 的直接替換（棄用旗標型別改用集合型別，範圍為兩個測試檔共六處，取自靜態分析工具的棄用告警） | 消費端＝那兩個測試檔自身（已觀測）；反例＝棄用告警是機械訊號，換 API 後是否消失可直接觀測；範圍已量 | **豁免**，發現當下即可寫定修法 |
| 共用工具（如共用元件）收斂為單一實例的重構提案 | 消費端＝其他 consumer 是否呼叫該工具，未觀測；反例未試；範圍雖已量但擴散面跨 consumer | **不豁免**，修法形狀移至 issue 決定 |
| 自我檢查範本的合法章節清單缺一項欄位 | 消費端＝該清單是否真被任何自檢消費、未列項如何處置，未觀測；反例未試；範圍為單一處（已量） | **不豁免**，擴散面屬會隨 sync 擴散的通用資產，修法形狀移至 issue 決定 |
...
唯一的接縫在規則 5 的優先級對應表：「高→當前版本（IMP Ticket）」容易被讀成「高優先級＝直接寫定修法的實作票」。正確讀法：**「高＝本版處理」，票型由上方三問的證據狀態決定，不由優先級決定**——高優先級而證據未備者，本版的處置是分析票（ANA），不是延後；分析票本身即是追蹤動作，不觸及規則 5「禁止以後續再說省略追蹤」的禁令，也不觸及 `.claude/rules/core/decision-trigger-binding.md`（本節不產生無 trigger 的延後：分析票本身就是 trigger）。

**現況**：本節為候選方案 A（純條文層），不改任何 CLI 或 hook。是否再加建票期 CLI 提示（候選方案 B：`ticket create` 於實作票型且帶來源欄位時提示三問）待 `#81` 區段討論後另行決定。票型閘門（候選方案 C：來源票非分析票的實作票一律先建分析票）已否決——會使健康形態的成本從一個句子放大為一張票，且對已量測樣本會使票池規模近乎倍增，與 `#81` 記錄的票池單調增長問題相斥。
