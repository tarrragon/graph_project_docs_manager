...
| 要求               | 正確範例                       | 錯誤範例              | 原因                                    |
| ------------------ | ------------------------------ | --------------------- | --------------------------------------- |
| 分層結構可拆解     | `v1.2.0-W03-021.4`             | `ticket_74_sub7`      | 前者可用分隔符拆成版本/wave/序號/子序號（portability-allow: ID 格式教學範例，非真實 ticket 引用） |
...
| `when`       | 何時啟動 / 何時截止 | `"v1.2.0 發佈前；依賴 W03-021 完成後啟動"`（portability-allow: ticket 啟動條件教學範例） | 產品行為時序     | 寫「當用戶點擊...系統會...」，描述產品行為而非 ticket 啟動條件   | 條件可被程式或人工驗證         |
