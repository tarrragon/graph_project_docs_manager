...
**把本表套到既有條文上的延伸教訓**：`writing-the-body.md`〈一則完整走查：兩個判準只有一個附了可執行動作〉示範把本表實際套用到一段既有條文的完整過程，含本表未涵蓋的兩則延伸教訓——「當兩個判準只有一個附了可執行動作，實際生效的永遠是有動作的那個」，以及「補上動作不是終點，還要回頭問合併規則」（合併規則若是 OR，生效的永遠是較嚴的一方，另一方形同未寫）。
