...
**適用邊界**：本節與下方〈常見問題排除〉的第一張表（上傳錯誤）預設 skill 經**上傳與打包**分發。走 git 同步的專案（本庫即是，見 `creating-and-adopting-skills.md` 的 Step 5）沒有上傳這個環節，因此三層測試中 Manual 與 Programmatic 兩層無管道、上傳錯誤那張表的三個訊息永遠不會出現。這類專案可執行的只有 Scripted 一層，而本檔未給它程序。
...
**本表只在經上傳分發時適用**；走 git 同步的專案不會遇到這三個訊息（同〈測試方法〉的適用邊界）。
