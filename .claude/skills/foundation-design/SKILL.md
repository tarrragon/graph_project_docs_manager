...
**一個維度的產物可以是複合的，不必擇一。** 產物不是原子的：同一維度常有一部分已存在、另一部分待建。填「已存在於 `main.go:136` 的色碼常數表 + 改寫產物：寬度截斷規則待落為文件」是正確答案；逼自己擇一時，選「已存在」會漏掉待建的部分，選「改寫產物」會重造已有的部分。

**monorepo 的產物欄按子專案分列。** 同一維度在各子專案的狀態常不同（一端已有 log 設施、另一端零設施），單一格會把落後那端的缺口吃掉。形式為「維度 × 子專案」的矩陣，每格獨立填四種答案之一。
...
| 形態 | 處置 |
|------|------|
| **權威是框架資產但執行本流程的 session 未安裝** | 產物欄填「待定：權威缺席」，**不得填「無」**。「沒有依據」與「不產出」是兩件事。建一張補該權威的票，該維度的地基票 `blockedBy` 它；或指定替代權威並記錄於被盤點 repo 的決策文件。**注意這一格不看被盤點的 repo**——它沒有框架是常態，不構成權威缺席 |
...
| 情境 | 判別（皆為可查證的事實） | 誰驅動 | 本 skill 的角色 |
|------|------------------------|-------|---------------|
| **無規格的既有實作** | 已有相當程度的實作，且行為只存在於程式碼（無規格文件、無設計記錄） | 本 skill | 既有 orchestration 皆不適用，它們假設已有規格或已在規劃波中。走〈老專案模式〉 |
| **規劃波進行中** | 該版提案的 `spec_refs` 或 `usecase_refs` 非空（`version-bootstrap` 的 pipeline 確實跑過），且該版尚未建票 | `version-bootstrap`（「建 Spec 骨架」與「地基波」兩步） | 交回，不重跑盤點 |
| **規劃波之後** | 該版票已建（不論是否已開工） | 無 | 本 skill 只用於補判未被盤點的維度，不重排既有票 |
| **其餘**（新專案規劃波之前、無 `version-bootstrap`、接手他人未完成的規劃波） | 以上皆不命中 | 本 skill | 依維度表逐維度定產物，產物交給 `version-sequencing` 排版本 |

**判別一律用可查證的事實，不用身份或進行式。** 「你是不是原作者」對 AI session 無從判定，且自己寫的專案同樣會缺規格；「實作進行中」會把「已建票未開工」這個最常見的靜止狀態排除在外。**「該版 `todolist.yaml` 填了 `proposals` 欄位」不等於規劃波進行中**——提案登記與 pipeline 執行是兩件事，誤判為第二列的代價是整個盤點被跳過，而下游不覆蓋 DevOps 與可觀測性。

**交回下游不等於下游覆蓋全部維度。** 實查 `version-bootstrap` 的步驟，`DevOps` 與 `可觀測性` 兩個維度在其全文 0 命中。交回前先為這兩個維度各建一張盤點票（產物欄可填「待定」），口頭明示不構成交接。

「無規格的既有實作」那一列是本 skill 無可替代的部分。
...
```
1 判定情境（無規格的既有實作 / 規劃波進行中 / 規劃波之後 / 其餘），由上而下先命中者適用
   → 規劃波進行中：為 DevOps 與可觀測性各建盤點票後交回 version-bootstrap，本流程結束
2 逐維度填產物欄（照預設 / 改寫產物 / 已存在於 X / 無），一格可複合，monorepo 按子專案分列
...
- [ ] **被盤點的 repo** 有執法載體（CI 或 hook 皆可）？步驟 5 依賴它
- [ ] **被盤點的 repo** 有決策文件（任何形式的持久記錄）？〈權威缺席時〉的形態 1 路徑 b、形態 4、以及下方的降級記錄都落在它上面

**缺項時進入降級模式，並記錄降級。** 記下缺哪一項、因此哪幾步在本專案不成立，並建一張補齊該基礎設施的票。缺 ticket 系統時仍可用於盤點與命名（前兩步），固化的機械檢查部分無載體。**連決策文件都沒有時，第一個動作是建立它**（一個 `docs/decisions.md` 即可），否則本流程的三處缺席處置全部無處落地。無降級記錄的使用不構成完成本流程。
...
本 skill 的判準歸納自單一作者的專案群，其中 Flutter 佔近半。已實跑驗證的形態：Flutter 桌面應用、Go CLI 單檔工具、多語言 SDK monorepo。**未經實跑驗證**：多人協作交接、非 Flutter 的 GUI 框架、單體遺留系統、有真實資料庫 schema 的服務。遇到這些形態時，維度表仍可用於提問，預設產物需要更多改寫。

〈維度與產物〉的兩個改寫案例取自 Go CLI 工具，屬**個案記錄而非推導範本**。該節的推導問句（「這個維度要防的失效，在本專案會長成什麼樣子」）才是可套用的部分。
