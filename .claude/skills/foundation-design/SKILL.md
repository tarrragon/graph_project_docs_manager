...
| **框架內確實無承接者** | **拆兩問，分開填**。產物層：依〈產物欄的五種合法答案〉四種作答——改寫產物／已存在於 X／無／待定（不含「照預設」，因無權威可提供預設產物）；選「無」仍須符合 `references/dimension-product-notes.md`〈判定「失效不存在」的理由〉的證明要求，不得僅因該物件目前不存在就代入。判準層：框架有無判斷它合格與否的依據，沒有則另標「無判準」並建框架缺口票。**兩者獨立**，「產物有、判準無」是常見狀態。現已知的判準缺口：**資料庫 seed**（本框架搜尋「種子資料／seed data／seeding」0 命中，因此無從判斷既有 seeder 的量級與冪等策略是否合格） |
...
| **規劃波之後** | **該版票已建**（不論是否已開工）。這是唯一判別依據——是否已有相當程度的既有實作與本列判定無關 | **仍由本 skill 驅動**：(1) 做什麼——逐維度補判未被既有票涵蓋的產物；(2) 不做什麼——不重排既有票。（這是真實 repo 最常見的狀態，非收手訊號） |
| **其餘** | 該版尚未建票，且不屬於前兩列——無相當程度的既有實作缺口，也未進入規劃波 pipeline | 依維度表逐維度定產物，產物交給 `version-sequencing` 排版本 |
...
**指名複核者的人工複核，驗收條件須寫三項**：複核者（先查該維度權威來源 skill／方法論是否已訂審查角色，已訂者用該角色；未訂時退至本框架既有審查代理人——兩者是先後查找順序而非任選其一；不得留白等待指派）、複核依據（該維度權威原文中最接近的判準條款，供複核者比對）、複核產出（一句合格／不合格的結論加理由，寫入該張地基票的 Solution）。機械檢查形態本身留給各維度依其權威自行設計，本 skill 不代為規定——權威若本身未提供機械檢查判準，屬〈權威缺席時〉「框架內確實無承接者」的判準層缺口，另標「無判準」建框架缺口票，不在步驟 5 解決。
...
**缺項時進入降級模式，並記錄降級。** 記下缺哪一項、因此哪幾步在本專案不成立。**缺的若是 ticket 系統**，步驟 4（產物落為地基票）與〈權威缺席時〉各處的建票動作在本專案皆不成立，改把應建票的內容直接記入降級記錄本身——不套用形態 4，形態 4 的觸發條件是 skill 缺席，不是 ticket 系統缺席。**缺的若是執法載體，不另建補齊票**——它就是 DevOps 維度的產物，會在步驟 4 一併落地，另建會使同一件事有兩張票。**連決策文件都沒有時，第一個動作是建立它**（一份進版控的決策記錄即可，位置依專案慣例），否則本流程的缺席處置全部無處落地。無降級記錄的使用不構成完成本流程。
