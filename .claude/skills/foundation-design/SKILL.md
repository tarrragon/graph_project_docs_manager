...
**本表列的是常見維度，不宣稱窮盡地基的全部外延。** 專案若有本表未涵蓋的地基工作（如協定契約、資料匯入匯出格式），照同一形式增列一行——依〈本 skill 不做什麼〉「指名既有權威」的做法，權威來源欄填一個 skill／方法論／可觀測性規則類別中的既有項目。**指名不到，才轉入下方〈權威缺席時〉「框架內確實無承接者」**：本表未列出這個維度，不等於指名不到既有權威，兩者是不同的判斷。
