...
「共用核心、差異點適配」的常見切法：底部 tab bar、push/pop 導航兩平台一致；back 手勢（iOS edge swipe + Android 系統鍵都要支援）、modal 呈現按平台適配。適配與否的判斷標準：系統行為類差異（back 手勢、系統鍵、modal dismiss 手勢）必適配 — 違反會直接打破使用者的肌肉記憶；視覺風格類差異（轉場動畫、圖示風格）可不適配 — 適配成本高於體驗增益時保持統一。
