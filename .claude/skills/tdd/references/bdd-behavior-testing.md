...
```
檢驗問句：
  Q 兩項訊號同時成立時，是否優先考慮拆分整個 Module？預期：否。優先依「步驟正確性由自己決定」拆出該步驟的單元測試；Module 是否需要拆分，留待單元測試拆出後、Given 設置是否仍然過長再判斷。
  Q 拆出單元測試後，原整合測試中對應該步驟的 Given 設置，是否仍須保留全部細節？預期：否。該步驟細節，已由拆出的單元測試覆蓋；原整合測試的 Given，可簡化為只保留串接所需的前置狀態。
```
