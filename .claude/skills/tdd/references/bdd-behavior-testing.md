...
| BDD 測試的 Given 設置過長（> 10 行 Mock） | Module 過大，考慮拆分 Module 而非加測試；套用不移動邊界動作後，原僅服務已拆出邏輯的 Given 設置可移除，移除後 Given 設置仍 > 10 行 Mock，才考慮拆分 Module（詳見下方〈多訊號同時成立時〉） |
| 步驟正確性由元件自身決定（非串接決定）    | 拆為單元測試（見 `doc-handoff-test-tier-assignment.md` 分工框架） |
| 步驟正確性由串接決定                      | 留在整合測試                                 |
| 步驟正確性由配置決定                      | 拆為參數化測試                               |

**多訊號同時成立時**：不移動 Module 邊界的動作（拆為 Domain／Value Object 單元測試、拆為單元測試、拆為參數化測試）優先於「拆分 Module」，依據〈Sociable vs Solitary 選擇框架〉「處方的第一順位都不是移動邊界」。先套用前者；套用後原整合測試中僅服務已拆出邏輯的 Given 設置可移除，移除後 Given 設置仍 > 10 行 Mock，才考慮拆分 Module。
