...
「涉及 UI」依 `.claude/skills/version-bootstrap/SKILL.md` Step 2〈UI 類判別〉呈現通道表判定：場景步驟的操作或預期結果落在第一列「圖形介面」即涉及 UI，第二、三列不觸發本檢查項；步驟未寫明呈現通道時，先回 doc 補寫通道再判定。判定只看呈現通道，不以該畫面是否本次新增或改動為條件。
