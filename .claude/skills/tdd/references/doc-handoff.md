...
以下以主成功場景其中一個步驟為例，展示「UC 主成功場景非空」與「涉及 UI」兩項檢查的判定方式。

【輸入】

```text
UC 主成功場景步驟：
  使用者於清單畫面選取一列，畫面切換為該列對應的詳情畫面
```
...
檢查項             | 判定 | 依據
UC 主成功場景非空 | 通過 | 該步驟同時具備動作（選取一列）與預期結果（畫面切換），非僅一句敘述
涉及 UI            | 通過 | 該步驟含使用者操作（選取），成立條件不是「新增或改動畫面」
```
...
```text
檢驗問句
  Q 判定「場景步驟完整」是否只要求步驟存在一句敘述？預期：否。動作與預期結果須同時具備，缺一即不算完整場景步驟。
  Q 判定「涉及 UI」是否以「本次是否新增或改動畫面」為門檻？預期：否。步驟含使用者操作即涉及 UI，與畫面是否為新增或改動無關。
```
...
使用者能感受到操作已完成   | 原文照抄，不轉換               | 「感受到」為主觀描述，無對應的可觀察計數或門檻值，判定不可量化
```
...
檢驗問句
  ...
  Q 原文僅有主觀感受描述且無可觀察對應時，是否仍勉強轉換為量化敘述？預期：否。判定為不可量化，原文照抄。
```
