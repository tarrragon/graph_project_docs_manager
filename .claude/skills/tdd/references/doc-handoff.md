...
```text
檢驗問句（各問句的判斷標準出自〈前置閘門〉表該檢查項的條件欄與界定段，本段不複述其內容，只查【產出】是否據以判定）
...
「按模組群組拆分」的模組群組以 spec 的 `domain` 為單位；單一 domain 產生的紅燈票超出**該專案訂的任務拆分閾值**時，再依架構層進一步切分。**閾值住在各專案自己的認知負荷規則裡，本 skill 不代訂**——沒有這類規則的專案，在第一次撞到「一張票裝不下」的時候把當下的票數記下來當起點，往後對照著調。<!-- portability-allow: 本框架的閾值表在 .claude/rules/core/cognitive-load.md，其餘專案取自己的 -->
