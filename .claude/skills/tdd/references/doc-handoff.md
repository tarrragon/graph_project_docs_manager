...
檢驗問句
  ...
  Q 步驟 2 的 When，未讀到的前序狀態，是否仍須列入 Given？預期：否。只列 When 會讀到的前序狀態，不須逐條列出全部前序狀態。
```
...
檢驗問句：
  ...
  Q 一個場景步驟宣告多個預期結果，且各自可獨立驗證時，是否仍只產出一個種子？預期：否。每個可獨立驗證的預期結果，各自拆為一個種子；本步驟因此拆為 5a、5b 兩個種子。
```

### GWT 種子與行為單元的分工邊界

GWT 種子產出後，由 Phase 1 設計者（非本銜接流程）判斷行為單元粒度。以下範例示範一條分工邊界：本銜接流程的產出物只到 GWT 種子，不含行為單元拆解。
