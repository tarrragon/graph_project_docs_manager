...
「涉及 UI」依 `.claude/skills/version-bootstrap/SKILL.md` Step 2〈UI 類判別〉呈現通道表判定：場景步驟的操作或預期結果落在第一列「圖形介面」即涉及 UI，第二、三列不觸發本檢查項；步驟未寫明呈現通道時，先回 doc 補寫通道再判定。判定只看呈現通道，不以該畫面是否本次新增或改動為條件。「圖形介面」與「非圖形的人讀呈現」在應用程式產出、版面固定的匯出檔上可能字面重疊；依 `.claude/skills/version-bootstrap/references/step-rationale.md`〈呈現通道表兩列的優先序〉，判準依查看／操作當下的呈現位置，不依版面豐不豐富——已離開本應用程式視窗、由外部程式另開檔案呈現者一律歸第二列（非 UI 類），即使版面由本應用程式產生。
...
涉及 UI            | 是，查矩陣     | 步驟的操作與預期結果落在呈現通道表第一列「圖形介面」；判定只看呈現通道，不看該畫面是否本次新增或改動
```
...
檢驗問句
  ...
  Q 判定「涉及 UI」是否以「本次是否新增或改動畫面」或「步驟是否含使用者操作」為門檻？預期：否。依呈現通道表，步驟的操作或預期結果落在第一列「圖形介面」即涉及 UI。
