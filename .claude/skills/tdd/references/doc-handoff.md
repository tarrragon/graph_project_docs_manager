...
| 跨 app process 邊界（系統權限對話框、原生檔案選擇器、外部 process、通知中心） | **平台驗證測試**（見 `references/layered-test-strategy.md` 跨邊界補位形態<!-- TODO: 該文件補位章節仍在補齊中，章節標題定案後回填精確連結 -->） | headless 的 `integration_test` 驅動不到系統原生 UI 或外部程式，判定者是平台而非 app 自己，需另一種測試形態 |
...
> BDD→單元拆分的完整判斷標準見 `references/bdd-behavior-testing.md`「BDD→單元拆分標準」章節。各層測試方法的詳細選擇、跨邊界步驟的具體測試形態見 `references/layered-test-strategy.md`<!-- TODO: 該文件補位章節仍在補齊中，章節標題定案後回填精確連結 -->。
...
**Version**: 1.1.3 — 〈整合測試 vs 單元測試分工〉判斷原則加第三值「平台決定」：既有「自己決定／串接決定／配置決定」三值皆假設判定權威落在 app process 之內，跨邊界步驟（呼叫離開 app process）三值都給不出答案；新增小節說明平台決定與既有三值是正交關係（先問「決定發生在 app process 內還是外面」，而非並列第四選項），分工決策框架表同步加一列指向 `layered-test-strategy.md` 的跨邊界補位形態（該文件補位章節尚在補齊中，暫以檔案層級引用 + TODO 標記，待章節標題定案後回填精確連結）。
