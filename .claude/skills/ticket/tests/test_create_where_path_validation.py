...
class TestValidateWhereFileToken:
    ...
    def test_prefix_src_equals_rejected(self):
        is_valid, _ = validate_where_file_token("src=.claude/x")
        assert is_valid is False

    def test_prefix_layer_equals_rejected(self):
        is_valid, _ = validate_where_file_token("layer=core")
        assert is_valid is False

    def test_prefix_equals_no_slash_rejected(self):
        # 整段即一個 key=value，無路徑分隔
        is_valid, _ = validate_where_file_token("where=foo")
        assert is_valid is False

    def test_reject_returns_hint(self):
        is_valid, hint = validate_where_file_token("src=.claude/x")
        assert is_valid is False
        assert hint
        # 修正提示應含剝除前綴後的建議路徑
    ...
    def test_claude_path_allowed(self):
        is_valid, _ = validate_where_file_token(".claude/skills/ticket/x.py")
        assert is_valid is True

    def test_src_path_allowed(self):
        is_valid, _ = validate_where_file_token("src/background/index.js")
        assert is_valid is True

    def test_relative_path_allowed(self):
        is_valid, _ = validate_where_file_token("a/b.py")
        assert is_valid is True

    def test_absolute_path_allowed(self):
        is_valid, _ = validate_where_file_token("/Users/x/project/file.py")
        assert is_valid is True

    def test_single_file_no_slash_allowed(self):
        is_valid, _ = validate_where_file_token("README.md")
        assert is_valid is True
    ...
    def test_equals_in_mid_segment_allowed(self):
        ...
        is_valid, _ = validate_where_file_token("src/a=b/c.py")
        assert is_valid is True

    def test_equals_in_filename_allowed(self):
        is_valid, _ = validate_where_file_token("docs/report=final.md")
        assert is_valid is True

    # --- 空字串視為合法（由上游過濾，不在此 reject） ---

    def test_empty_token_allowed(self):
        is_valid, _ = validate_where_file_token("")
        assert is_valid is True
...
class TestValidateWhereFiles:
    ...
    def test_all_valid_returns_no_errors(self):
        errors = validate_where_files([".claude/x", "src/y", "a/b.py"])
        assert errors == []

    def test_dirty_token_collected(self):
        errors = validate_where_files(["src=.claude/x", "src/y"])
        assert len(errors) == 1
        assert "src=.claude/x" in errors[0]

    def test_multiple_dirty_tokens_all_collected(self):
        errors = validate_where_files(["src=.claude/x", "layer=core"])
        assert len(errors) == 2

    def test_empty_list_no_errors(self):
        assert validate_where_files([]) == []
