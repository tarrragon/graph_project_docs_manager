...
驗證 parse_acceptance_items / split_unescaped 兩 helper：
- 內文含分隔符時的拆條行為與警告
- 反斜線跳脫保留字面分隔符不拆條
- 多值與去空白/去空項處理
...
from ticket_system.lib.acceptance_parser import (
    ACCEPTANCE_SEP as SEP,
...
def test_single_value_no_separator_keeps_one_item():
    acceptance, warnings = parse_acceptance_items(["驗收條件 A"])
...
def test_separator_splits_into_multiple_items():
    raw = f"條件一{SEP}條件二"
    acceptance, warnings = parse_acceptance_items([raw])
...
def test_multiple_acceptance_args_each_evaluated():
    ...
    acceptance, warnings = parse_acceptance_items([raw_a, raw_b])
...
def test_whitespace_trimmed():
    raw = f"  前後空白  {SEP}  另一條  "
    acceptance, _ = parse_acceptance_items([raw])
