...
參考規範:
- .claude/skills/compositional-writing/references/writing-code-comments.md（portability-allow: 跨 skill 引用，comment-qa-hook 遷至 dart-comment-traceability 後，通用註解撰寫規範仍由 compositional-writing 持有）
- docs/event-driven-architecture-design.md（可選：consumer 若無此文件，報告省略此列，不影響其餘檢查，見 find_related_spec_files() 與 generate_report()）
...
def log_message(logger, message: str = None):
    ...
    相容既有兩種呼叫慣例：`log_message(logger, message)`（呼叫端持有 logger 參照）
    與單引數呼叫 `log_message(message)`（should_process_file /
    extract_functions_with_parser / extract_dart_widgets / find_related_work_log /
    extract_design_solution / save_report 這幾個輔助函式未接收 logger 參數，
    僅有訊息字串）。單引數呼叫退回模組級同名 logger——與 main() 呼叫
    setup_hook_logging("comment-qa-hook") 建立的 logger 為同一 named-logger
    實例（Python logging 對同名 getLogger 回傳快取單例），共用其已掛載的 handler。
    """
    if message is None:
        logging.getLogger("comment-qa-hook").info(logger)
        return
    logger.info(message)
...
def generate_report(
    ...
    # 架構文件為可選引用：consumer 無此文件時省略此列，不影響其餘檢查（PROJECT_ROOT
    # 相對路徑存在性檢查，本專案與 flutter_balance 皆無此文件）
    if (PROJECT_ROOT / "docs/event-driven-architecture-design.md").exists():
        reference_lines.append("- `docs/event-driven-architecture-design.md` - 事件驅動架構規範")
    reference_lines.extend([
        "- 註解必須記錄「為什麼」而非「做什麼」",
        "- 註解必須包含需求來源和工作日誌追溯",
        "",
    ])
    report_lines.extend(reference_lines)
    report_lines.extend([
        "---",
        f"報告生成時間: {timestamp}",
