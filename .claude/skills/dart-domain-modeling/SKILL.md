...
- 「介面隔離 + 語意化方法優先於直接操作狀態」是本 skill 判準的上位原則。狀態管理層曾有對應的 `dart-provider-architecture` skill，因其參考範例綁定單一專案的檔案路徑而移除；需要 Riverpod 層規範時，以所在專案 `lib/core/providers`（或等價目錄）的既有慣例為準，不套用他專案的範例
