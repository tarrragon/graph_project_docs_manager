...
系統進入卸載（load shedding）時的額外訊號 — 每個仲裁決策的可觀測事件、被丟棄請求的留痕粒度、告警綁狀態變遷而非「佔用量高」— 判斷標準見事件流負載仲裁方法論〈可觀測性〉、欄位範本與訪談流程見 `event-flow-arbitration-design` skill，以本維度的 structured log 與 request id 為前提。
