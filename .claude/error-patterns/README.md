...
| DOC-GPD-006 | 帶條件的決策被壓進表格欄位，條件掉落使結論的決策語意改變 | 高 | 0.1.0 |
| DOC-GPD-007 | 介面文件只示範一種參數形式，讀者將範例讀為契約全集因而推論出不存在的限制 | 中 | 0.1.0 |
| DOC-GPD-008 | 從單一實例外推通則時，分類的例示清單沿用來源實例的直覺而未逐一回代判準鍵，例示與鍵矛盾且讀者依例示分類 | 高 | 0.1.0 |
| DOC-GPD-009 | 對表述歧義加一條警告而非改句子結構，複驗顯示歧義原封不動 | 中 | 0.1.0 |
| DOC-GPD-010 | 規範條文的效力取決於讀者是否走到它，不在執行路徑上的條文措辭再清楚也零生效 | 高 | 0.1.0 |
...
| ARCH-GPD-001 | 互動回饋掛在服務的成功路徑上，服務越不可用回饋越少 | 高 | 0.1.0 |
...
| PC-GPD-008 | 未填樣板通過數量型檢查，因為它形狀正確而內容空白 | 中 | 0.1.0 |
| PC-GPD-009 | 驗證器驗的維度與缺陷所在的維度不同軸，每次都是「查了，而且通過了」 | 高 | 0.1.0 |
| PC-GPD-010 | 編輯使用中的工具，危險窗口是「還沒改完」而非「改壞了」 | 中 | 0.1.0 |
...
| PC-GPD-011 | 過濾產物活得比它的問題久——子集被拿去回答另一個問題 | 高 | 0.1.0 |
| PC-GPD-012 | 共識不構成查證——繼承來的前提在交叉檢查中免於審查 | 高 | 0.1.0 |
| PC-GPD-013 | 繞道路徑成功，使共用資源的故障失去唯一的診斷訊號 | 高 | 0.1.0 |
| PC-GPD-014 | 正確結論配錯誤理由當下無症狀，要等前提改變才暴露 | 高 | 0.1.0 |
| PC-GPD-015 | 引用了對的權威來源，但它回答的不是被問的那個問題 | 高 | 0.1.0 |
| PC-GPD-016 | 審查中採納的修法產物進入下一輪時不帶標記，被當成既有前提而不再受檢 | 高 | 0.1.0 |
| PC-GPD-017 | 規範類文件的禁令只被問效力不被問代價，誤傷的合規設計無 frame 負責 | 高 | 0.1.0 |
| PC-GPD-019 | 形狀相符終止查證——附近有個東西長得像答案，於是不再往下讀 | 高 | 0.1.0 |
| PC-GPD-020 | 明示的意圖與明示的失敗，在可觀測面與預設或成功不可區分 | 高 | 0.1.0 |
| PC-GPD-021 | 教訓段落生成未經查證的機制解釋——體裁獎勵因果完整而非證據標示 | 高 | 0.1.0 |
| PC-GPD-022 | 跨間接層的驗證錯誤指向被檢查層而非損壞層（git worktree 形態已最小重現；跨載體泛化仍為假說） | 中 | 0.1.0 |
| PC-GPD-023 | 先把尚未分配的識別符寫進文件、後才向共用計數器申請，並行下該引用會指向他人的物件 | 中 | 0.1.0 |
| PC-GPD-024 | 變更記錄照修改計畫寫而非照 diff 寫，同輪的無關改善為不存在的修法背書 | 高 | 0.1.0 |
| PC-GPD-025 | 以參考答案對照量測文件是否足夠時，猜中被計為非缺口 | 高 | 0.1.0 |
| PC-GPD-026 | 派發裁示指名一個空白卻不填內容，執行者自行決定並與既有先例矛盾 | 高 | 0.1.0 |
