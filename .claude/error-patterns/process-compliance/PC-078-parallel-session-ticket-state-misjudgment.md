...
## 邊界：本模式不涵蓋「同儕確認停擺後如何推進」

本模式的觸發訊號是**工作區出現非本 session 引起的狀態變化**，處方是停手先問，防的是擅動他人活躍工作。

另一個相鄰情境的觸發訊號不同——**自身目標範圍內的某票由同儕持有且遲遲未推進**，此時要決定的不是「該不該動這個變更」而是「該不該接手這張票」。該情境的判準見 `.claude/pm-rules/parallel-dispatch.md`「跨 session 同儕沉默時的接管判準」，其中包含一條與本模式互補的發射方義務：持有票而預期長時間無產出時先 `claim` 留痕，使 `started_at` 成為存在的訊號——這正是本模式教訓 1 所述無歧義訊號的主動運用。

兩者的分工：本模式管「看到不明變化時不要擅動」，該章節管「確認對方停擺後如何推進」。讀錯情境會套錯處方。
