...
## 變體：診斷已在讀者自己的 context 裡，仍在同一份文件複製該形態

本檔原案例的距離是「ANA 主題」與「該 ANA 的收尾動作」之間。存在一個距離更短、因此更值得記的變體：**作者讀過一份文件中對某失效形態的診斷，隨後在同一份文件的新增段落製造出該形態**。

**案例**：一份 append-only 決策記錄中已有一節診斷「某前提垮了，掛在它下面的結論全部要重估，其中有些其實不依賴它」，並明確指出這種錯誤依附會導致連鎖誤判。同一份文件的作者在該節下游新增一節架構決策，把五項結論全部掛在一條新原則之下並宣稱「皆由此原則推得」——而該宣稱被同節下方五個論證段落中的三個直接否證：其中兩項的論證通篇未提及該原則。多視角審查的替代方案視角判定：新節在建立結論時，複製了同一份文件剛診斷出的失效形態。

**為何比原案例更難防**：原案例中作者要跨越「主題」與「流程」兩個層面才會失手；本變體中診斷與複製發生在同一份文件、同一天、同一位作者，且該診斷內容確實已被讀取並在新節中被引用。**讀過不構成防護**——診斷被當成「關於過去那件事的記錄」消費，而非「對當前產出的判準」。

**辨識訊號**（與原案例的訊號互補，不重疊）：

| 訊號 | 說明 |
|------|------|
| 新增內容引用了同檔既有的失效診斷 | 引用即證明作者讀過，排除「不知道」 |
| 新增內容含「皆由 / 全部依 / 一律基於 ⟨單一原則⟩」式的總括句 | 總括句是依附關係的宣告，而依附關係正是被診斷的形態 |
| 該總括句未逐項核對 | 核對成本低（讀自己寫的論證段），但總括句通常在論證段之前寫成 |

**防護（可在撰寫當下自檢，成本為一次逐項讀回）**：寫下「這 N 項皆由原則 P 推得」之後，逐項讀自己給的論證，標記每項是否真的用到 P。做不到全部標記為「是」，就改寫為指名射程的句子（「第一項與第三項由 P 推得；第二項為併發控制、第五項為輸入驗證，兩者不依賴 P」）。**這是純文件成本，但它決定下一次前提變動時的重估範圍是否正確**——假的依附關係會在兩個方向出錯：既牽連不該重估的結論，也會讓一條正當的窄修改誤以為必須先推翻總原則。

---

**Last Updated**: 2026-09-10
**Version**: 1.1.0 — 新增「變體：診斷已在讀者自己的 context 裡」：距離更短的形態（同一份文件、同一天、同一作者，且診斷已被新節引用），附三項與原案例互補的辨識訊號與一項撰寫當下可執行的自檢。原案例與防護三層不變。
