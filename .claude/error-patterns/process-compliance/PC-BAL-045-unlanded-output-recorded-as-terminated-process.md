...
1. **清單覆蓋缺口。** `ListAgents` 回傳 peer session 但不含 in-process teammate。若只信 `ListAgents`，會得到「無閒置代理人」的假結論；本例是用戶另從代理人樹狀清單指認才發現兩個 idle 代理人。
...
| 無閒置代理人可回收 | `ListAgents` 與 teams config `members[]` 兩個觀測面都查；後者證明曾派發，不證明存活 | 「ListAgents 未列出，teams config 內的 in-process 成員未逐一確認」 |
...
- 清點活躍代理人時兩個觀測面都要看。單一來源的空清單不構成「沒有」的證據；`PC-166` 防護 E 同樣要求以第二個觀測面交叉驗證。
